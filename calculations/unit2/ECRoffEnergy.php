    ...
    $where = array(
     ...
     foreach ($docs as $document) {
         ...
         foreach ($document as $key => $value) {
              ...
              $arr[] = $document['GW1_U2_ActiveEnergy_Delivered_kWh'];
             }
             if(!empty($document['GW1_U3_ActiveEnergy_Delivered_kWh'])){
              $arr1[] = $document['GW1_U3_ActiveEnergy_Delivered_kWh'];
             }
             if(!empty($document['GW1_U4_ActiveEnergy_Delivered_kWh'])){
              $arr2[] = $document['GW1_U4_ActiveEnergy_Delivered_kWh'];
             } 
             if(!empty($document['GW1_U5_ActiveEnergy_Delivered_kWh'])){
              $arr3[] = $document['GW1_U5_ActiveEnergy_Delivered_kWh'];
             }  
             if(!empty($document['GW3_U3_ActiveEnergy_Delivered_kWh'])){
              $arr4[] = $document['GW3_U3_ActiveEnergy_Delivered_kWh'];
             } 
            if(!empty($document['GW3_U4_ActiveEnergy_Delivered_kWh'])){
              $arr5[] = $document['GW3_U4_ActiveEnergy_Delivered_kWh'];
             }
             if(!empty($document['GW3_U5_ActiveEnergy_Delivered_kWh'])){
                $arr6[] = $document['GW3_U5_ActiveEnergy_Delivered_kWh'];
               }
          }
         $index++;        
     }
     ...
     $data = $U_1+$U_2+$U_3+$U_4+$U_5+$U_6+$U_7;
