...
function fetchWeekly($monthStart,$monthEnd, $tag_values,$numberOfMeters){
  ...
  $cursor = $collection->aggregate([
    ['$project' => ['UNIXtimestamp' => 1, $tag_values[0] => 1,$tag_values[1] => 1,$tag_values[2] => 1,$tag_values[3] => 1,$tag_values[4] => 1,$tag_values[5] => 1,$tag_values[6] => 1]],
    ['$match' => ['UNIXtimestamp' => ['$gte' => $monthStartUnix,'$lte' => $monthEndUnix ]]],
    ['$project' => [
      'UNIXtimestamp'=>1,
      'date' => ['$add' => [['$toDate' => ['$multiply' => ['$UNIXtimestamp', 1000]]], 18000000]],
      'u1'=>'$'.$tag_values[0],'u2'=>'$'.$tag_values[1],'u3'=>'$'.$tag_values[2],'u4'=>'$'.$tag_values[3],'u5'=>'$'.$tag_values[4],
      'u6'=>'$'.$tag_values[5],'u7'=>'$'.$tag_values[6],
       ]],
    ['$project' => [
        'UNIXtimestamp'=>1,
  ...
  ['$project' => [
    ...
    'kWh3'=>['$subtract'=>['$lastRead4.u4','$firstRead4.u4']],
    'kWh4'=>['$subtract'=>['$lastRead5.u5','$firstRead5.u5']],
    'kWh5'=>['$subtract'=>['$lastRead6.u6','$firstRead6.u6']],
    'kWh6'=>['$subtract'=>['$lastRead7.u7','$firstRead7.u7']],
]],
  ]);
  $docs = $cursor->toArray();
  $index = 0;
...
function fetchHourly($date, $tag_values,$numberOfMeters){
  ...
  $cursor = $collection->aggregate([
    ...
    'u1'=>'$'.$tag_values[0],'u2'=>'$'.$tag_values[1],'u3'=>'$'.$tag_values[2],'u4'=>'$'.$tag_values[3],'u5'=>'$'.$tag_values[4],'u6'=>'$'.$tag_values[5],'u7'=>'$'.$tag_values[6],
     ]],
...
function fetchDayWise($date, $tag_values,$numberOfMeters){
  ...
  $cursor = $collection->aggregate([
    ...
    foreach ($docs as $document) {
      ...
      $array[] = array(
        'meter' =>  $date,
        'Old Boiler' => $document['kWh0'],
       );
       $array[] = array(
        'meter' =>  $date,
        'Juice Room' => $document['kWh1'],
       );
       $array[] = array(
        'meter' =>  $date,
        'New Boiler' => $document['kWh2'],
       );
       $array[] = array(
        'meter' =>  $date,
        'Sugar Room' => $document['kWh3'],
       );
       $array[] = array(
        'meter' =>  $date,
        'Syrup Room' => $document['kWh4'],
       );
       $array[] = array(
        'meter' =>  $date,
        'Sugar Dissolving' => $document['kWh5'],
       );
       $array[] = array(
        'meter' =>  $date,
       'Shrink Tunnel' => $document['kWh6']
      );
     $index++;
    }
   return $array;
...
elseif ($value == 'Last Week') {
  ...
  for ($i=0;$i<=6;$i++){ $info[$i]= fetchDayWise( $start_date, $tag_values,$numberOfMeters);
    $start_date = date('Y-n-j', strtotime($start_date . ' +1 day'));}
    $g=0;
// echo json_encode($info[14]);
for($x=0;$x<sizeof($info);$x++){
...
//  var_dump($document);
  $monthly[] = array(
    ...
    'value3' => $document['kWh3'],
    'value4' => $document['kWh4'],
    'value5' => $document['kWh5'],
    'value6' => $document['kWh6'],
   );
   $index++;
}
// var_dump($monthly);
...
for ($i = 0; $i <=(sizeof($monthly)-1); $i++) { 
  ...
  elseif ($monthly[$i]['month'] == 12) {
   $orgname = 'Dec';
    ...
    $count4 = $monthly[$i]['value3'];
    $count5 = $monthly[$i]['value4'];
    $count6 = $monthly[$i]['value5'];
    $count7 = $monthly[$i]['value6'];
