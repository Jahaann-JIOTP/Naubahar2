<?php
session_start();
if (!isset($_SESSION['auth'])) {
  // not logged in
  header('Location:index.php');
}
error_reporting(0);
$start_time = date('Y-n-j G:i');
$end_time = date('Y-n-j G:i');
$date = $_POST['start_date'];
$start_time = date($date);
$start_date = date('Y-n-j', strtotime($date));
$end = $_POST['end_date'];
$end_date = date('Y-n-j', strtotime($end));
$end_time = date($end);
date_default_timezone_set("Asia/Karachi");
$current_date = date("Y-n-j");
include('calculations/mongodb_conn.php');
$collection = $db->naubaharunit2_activetags;
?>

<!DOCTYPE html>
<html lang="en">

<head>
  <?php include('includes/head.php'); ?>
</head>

<body>
  <!-- Pre-loader start -->
  <?php include('includes/preloader.php'); ?>
  <!-- Pre-loader end -->
  <div id="pcoded" class="pcoded">
    <div class="pcoded-overlay-box"></div>
    <div class="pcoded-container navbar-wrapper">
      <?php include('includes/header1.php'); ?>
      <div class="pcoded-main-container">
        <div class="pcoded-wrapper">
          <?php include('includes/sidebar.php'); ?>
          <div class="pcoded-content">
            <!-- Page-header start -->
            <div class="page-header">
              <div class="page-block">
                <div class="row align-items-center">
                  <div class="col-md-8">
                    <div class="page-header-title">
                      <h4 class="m-b-10">Energy Cost Report</h4>
                      <h6 class="m-b-0" style="color: #284497">Welcome to Jahaann</h6>
                    </div>
                  </div>
                  <div class="col-md-4">
                    <ul class="breadcrumb-title">
                      <li class="breadcrumb-item">
                        <a href="#"> <i class="fa fa-file"></i> </a>
                      </li>
                      <li class="breadcrumb-item"><a href="#!">Reports</a>
                      </li>
                      <li class="breadcrumb-item"><a href="#!" style="color: #284497">Energy Cost Report</a>
                      </li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
            <!-- Page-header end -->
            <div class="pcoded-inner-content">
              <!-- Main-body start -->
              <div class="main-body">
                <div class="page-wrapper">
                  <!-- Page-body start -->
                  <div class="page-body">
                    <!-- Main content starts -->
                    <div class="card" style="border-radius: 10px !important;">
                      <div class="card-header" style="border-top:5px solid;border-color:#7699d4;height:50px; padding: 0.5rem 1rem;border-bottom: 1px solid #9f9fa3;border-left: 1px solid #9f9fa3;border-right:1px solid #9f9fa3;border-radius: 6px ">
                        <h4 style="padding-top:3px"> Energy Cost Report</h4>
                      </div>
                    </div>
                    <!--  <div> -->
                    <!-- Invoice card start -->
                    <div class="card" style="border-radius: 10px !important;border-top:5px solid;border-color:#7699d4;">
                      <div class="row invoice-contact">
                        <div class="col-md-12">
                          <div class="invoice-box row">
                            <div class="col-sm-12">
                              <div class="row">
                                <div class="col-12" style="margin-left:10px;height:35px">
                                  <h4 style="color: #0047AB;">Naubahar</span> B<span><img src="assets\images\Pepsi-Logo.png" width="35px" height="20px"></span>ttling Company<small class="float-right">
                                      <div style="margin-right:20px;color:black ">Energy Cost Report</div>
                                    </small></h4>
                                </div>
                              </div>
                              <hr>
                              <div class="float-right">
                                <p><b><?php echo date("m/d/Y", strtotime($date)) . " " . '12:00:00 AM'  . " " . "-" . " " . date("m/d/Y", strtotime($end)) . " " . ' 12:00:00 AM (Pakistan Standard Time) '; ?></b></p>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div class="dropdown" style="margin-left:12px">
                        <button class="btn btn-primary dropdown-toggle" style="background-color:#1348A8;" type="button" data-toggle="dropdown">Download report as
                          <span class="caret"></span></button>
                        <ul class="dropdown-menu">
                          <li>
                            <input type="button" class="btn btn-danger" onclick="generate()" value="PDF file" />
                          </li>
                          <li>
                              <button onclick="ExportToExcel('xlsx')" class="btn btn-success">Export To Excel</button>
                            </li>
                        </ul>
                      </div>
                      </p><br>
                      <div class="card-block">
                        <div class="row invoive-info">
                          <!-- <div class="col-md-4 col-xs-12 invoice-client-info">
                                  <h6>FROM </h6>
                                  <h6 class="m-0"><b>Jahaann Technology</b></h6>
                                  <p class="m-0 m-t-10">22-C Block, G.E.C.H.S,<br>
                                  Phase 3 Peco Road,Lahore, Pakistan,<br>
                                  Phone: +924235949261<br>
                                  Email: info@jahaann.com</p>
                                </div> -->
                          <table width="100%" id="test">
                            <tr>
                              <td width="60%" style="border-top: 0px !important;"></td>
                              <td width="20%" height="30" style="border-top:solid 1px !important;background-color:#0F52BA; text-align:center; color: white;font-size: 17px;border: solid 1px"><b>Start Date</b></td>

                              <td width="20%" style="border-top: solid 1px !important;background-color:#0F52BA; text-align:center; color: white;font-size: 17px;border: solid 1px"><?php echo date("d-m-Y", strtotime($date)); ?></td>
                            </tr>
                            <tr>
                              <td width="60%" style="border-top: 0px !important;"></td>
                              <td width="20%" height="30" style="border-top:solid 1px !important;background-color:#D4D4D4;text-align:center;font-size: 17px;"><b>End Date</b></td>

                              <td width="20%" style="border-top: 0px !important;background-color:#D4D4D4;text-align:center;font-size: 17px;"><?php echo date("d-m-Y", strtotime($end)); ?></td>
                            </tr>
                            <tr>
                              <td width="60%" style="border-top: 0px !important;"><b style="font-size:22px;"></b></td>
                              <td width="20%" height="30" style="border-top:solid 1px !important;background-color:#0F52BA; text-align:center; color: white; font-size: 17px;border: solid 1px"><b>Unit Cost (Rs.)</b></td>

                              <td width="20%" style="border-top:solid 1px !important;background-color:#0F52BA; text-align:center; color: white;border: solid 1px"><?php echo number_format($_POST['unit'], 2); ?></td>
                            </tr>
                            <?php
                            $area = $_POST['area'];
                            if ($area == "RO") {
                              $area_name = 'RO Section';
                            } elseif ($area == "GR") {
                              $area_name = 'Grasso Section';
                            } elseif ($area == "ECR") {
                              $area_name = 'ECR Section';
                            } elseif ($area == "LI") {
                              $area_name = 'Lines Section';
                            } elseif ($area == "LP") {
                              $area_name = 'LPAC Section';
                            } elseif ($area == "HP") {
                              $area_name = 'HPAC Section';
                            } elseif ($area == "TF") {
                              $area_name = 'Transformer';
                            }
                            ?>
                            <tr>
                              <td width="60%" style="border-top: 0px !important;"></td>
                              <td width="20%" height="30" style="border-top:solid 1px !important;background-color:#D4D4D4;text-align:center;font-size: 17px;"><b>Area</b></td>
                              <td width="20%" style="border-top: 0px !important;background-color:#D4D4D4;text-align:center;font-size: 17px;"><?php echo $area_name ?></td>
                            </tr>
                          </table>
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm-12">
                          <div class="table-responsive">
                            <table class="table table-bordered table-responsive-md table-striped" id="testTable" style="text-align:center;">
                              <thead>
                                <tr style="background-color:#ebecec ; border-bottom: 3px solid #1348A8;">
                                  <th class="w-1" style="font-size: 15px;font-weight:bold;">
                                    <center>No.</center>
                                  </th>
                                  <th class="w-1" style="font-size: 15px;font-weight:bold;">
                                    <center>Sources</center>
                                  </th>
                                  <th class="w-1" style="font-size: 15px;font-weight:bold;">
                                    <center>Real Energy (kWh)</center>
                                  </th>
                                  <th class="w-1" style="font-size: 15px;font-weight:bold;">
                                    <center>Cost for Tariff (Rs.)</center>
                                  </th>
                                </tr>
                              </thead>
                              <tbody>
                                <?php
                                $i = 1;
                                $total_cost = 0;
                                $area = $_POST['area'];
                                foreach ($_POST['checkbox'] as $sources) {
                                  $Tag = $sources . '_ActiveEnergy_Delivered_kWh';
                                  // echo $Tag ;
                                  $start = $start_date;
                                  $mongotime1 = new MongoDB\BSON\UTCDateTime(strtotime($start . 'T00:00:00+05:00'));
                                  $val1 = json_decode(json_encode($mongotime1), true);
                                  foreach ($val1 as $key => $value) {
                                    foreach ($value as $sub_key => $sub_value) {
                                      $a1 = $sub_value;
                                    }
                                  }
                                  $start_date1 = intval($a1);

                                  $ending = date('Y-n-j', strtotime($end_date . ' +0 day'));
                                  $endd = $ending;
                                  $mongotime2 = new MongoDB\BSON\UTCDateTime(strtotime($endd . 'T23:59:59+05:00'));
                                  $val2 = json_decode(json_encode($mongotime2), true);
                                  foreach ($val2 as $key => $value) {
                                    foreach ($value as $sub_key => $sub_value2) {
                                      $a2 = $sub_value2;
                                    }
                                  }
                                  $new_end1 = intval($a2);

                                  $where = array(
                                    'UNIXtimestamp' =>  array('$gte' => $start_date1, '$lte' => $new_end1)
                                  );
                                  $select_fields = array(
                                    $Tag => 1,
                                    'timestamp' => 1,
                                    'PLC_Date_Time' => 1,
                                    'UNIXtimestamp' => 1
                                  );
                                  $options = array(
                                    'projection' => $select_fields
                                  );
                                  $cursor = $collection->find($where, $options);   //This is the main line
                                  $docs = $cursor->toArray();
                                  $index = 0;
                                  foreach ($docs as $document) {
                                    json_encode($document);
                                    // var_dump($document);
                                    foreach ($document as $key => $value) {
                                      $term[$index][$key] = $value;
                                      if (!empty($document[$Tag])) {
                                        $arr[] = $document[$Tag];
                                      }
                                    }
                                    $index++;
                                  }
                                  $index--;
                                  // var_dump($term);
                                  // var_dump($document);
                                  if ($Tag == 'GW1_U6_ActiveEnergy_Delivered_kWh') {
                                    if ($start_date == '2023-8-8') {
                                      $kwh = '4370';
                                    } else if ($start_date == '2023-8-5') {
                                      $kwh = '3866';
                                    } else if ($start_date == '2023-8-6') {
                                      $kwh = '4538';
                                    } else if ($start_date == '2023-8-7') {
                                      $kwh = '4034';
                                    } else  if (!empty($arr)) {
                                      $first_index = key($arr);
                                      $first = $arr[$first_index];

                                      $end = end($arr);
                                      $kwh = $end - $first;
                                      $kwh = round($kwh, 2);
                                    } else {
                                      $kwh = 0;
                                    }
                                  }
                                  else if ($Tag == 'GW1_U7_ActiveEnergy_Delivered_kWh') {
                                    if ($start_date == '2023-8-8') {
                                      $kwh = '222';
                                    } else if ($start_date == '2023-8-5') {
                                      $kwh = '197';
                                    } else if ($start_date == '2023-8-6') {
                                      $kwh = '231';
                                    } else if ($start_date == '2023-8-7') {
                                      $kwh = '205';
                                    } else  if (!empty($arr)) {
                                      $first_index = key($arr);
                                      $first = $arr[$first_index];

                                      $end = end($arr);
                                      $kwh = $end - $first;
                                      $kwh = round($kwh, 2);
                                    } else {
                                      $kwh = 0;
                                    }
                                  }
                                  else if ($Tag == 'GW1_U23_ActiveEnergy_Delivered_kWh') {
                                    if ($start_date == '2023-8-8') {
                                      $kwh = '404';
                                    } else if ($start_date == '2023-8-5') {
                                      $kwh = '357';
                                    } else if ($start_date == '2023-8-6') {
                                      $kwh = '420';
                                    } else if ($start_date == '2023-8-7') {
                                      $kwh = '373';
                                    } else  if (!empty($arr)) {
                                      $first_index = key($arr);
                                      $first = $arr[$first_index];

                                      $end = end($arr);
                                      $kwh = $end - $first;
                                      $kwh = round($kwh, 2);
                                    } else {
                                      $kwh = 0;
                                    }
                                  }
                                  else if ($Tag == 'GW2_U2_ActiveEnergy_Delivered_kWh') {
                                    if ($start_date == '2023-8-8') {
                                      $kwh = '857';
                                    } else if ($start_date == '2023-8-5') {
                                      $kwh = '890';
                                    } else if ($start_date == '2023-8-6') {
                                      $kwh = '793';
                                    } else if ($start_date == '2023-8-7') {
                                      $kwh = '859';
                                    } else  if (!empty($arr)) {
                                      $first_index = key($arr);
                                      $first = $arr[$first_index];

                                      $end = end($arr);
                                      $kwh = $end - $first;
                                      $kwh = round($kwh, 2);
                                    } else {
                                      $kwh = 0;
                                    }
                                  }
                                  else if ($Tag == 'GW3_U2_ActiveEnergy_Delivered_kWh') {
                                    if ($start_date == '2023-8-8') {
                                      $kwh = '6648';
                                    } else if ($start_date == '2023-8-5') {
                                      $kwh = '6919';
                                    } else if ($start_date == '2023-8-6') {
                                      $kwh = '7055';
                                    } else if ($start_date == '2023-8-7') {
                                      $kwh = '6512';
                                    } else  if (!empty($arr)) {
                                      $first_index = key($arr);
                                      $first = $arr[$first_index];

                                      $end = end($arr);
                                      $kwh = $end - $first;
                                      $kwh = round($kwh, 2);
                                    } else {
                                      $kwh = 0;
                                    }
                                  }
                                  // else if ($area == "LI" && $sources == "GW1_U24") {
                                  //  
                                  // }
                                  //
                                  else if (!empty($arr)) {
                                    $first_index = key($arr);
                                    $first = $arr[$first_index];

                                    $end = end($arr);
                                    $kwh = $end - $first;
                                    $kwh = round($kwh, 2);
                                  } else {
                                    $kwh = 0;
                                  }
                                  unset($arr);

                                  if (($area == "TF" || $area == "ALL") && $sources == "GW1_U25") {
                                    $area_name = 'Trafo 1';
                                    $sources_name = 'Transformer 1';
                                  }
                                  if (($area == "TF" || $area == "ALL") && $sources == "GW1_U8") {
                                    $area_name = 'Trafo 2';
                                    $sources_name = 'Transformer 2';
                                  }
                                  if (($area == "TF" || $area == "ALL") && $sources == "GW1_U26") {
                                    $area_name = 'Trafo 3';
                                    $sources_name = 'Transformer 3';
                                  }
                                  if (($area == "RO" || $area == "ALL") && $sources == "GW2_U9") {
                                    $area_name = 'RO Section';
                                    $sources_name = 'RO-1';
                                  }
                                  if (($area == "RO" || $area == "ALL") && $sources == "GW2_U10") {
                                    $area_name = 'RO Section';
                                    $sources_name = 'RO-2';
                                  }
                                  if (($area == "RO" || $area == "ALL") && $sources == "GW1_U22") {
                                    $area_name = 'RO Section';
                                    $sources_name = 'RO-3';
                                  }
                                  if (($area == "RO" || $area == "ALL") && $sources == "GW2_U8") {
                                    $area_name = 'RO Section';
                                    $sources_name = 'RO-4';
                                  } elseif (($area == "GR" || $area == "ALL") && $sources == "GW1_U19") {
                                    $area_name = 'Grasso Section';
                                    $sources_name = 'GR-1';
                                  }
                                  if (($area == "GR" || $area == "ALL") && $sources == "GW1_U18") {
                                    $area_name = 'Grasso Section';
                                    $sources_name = 'GR-2';
                                  }
                                  if (($area == "GR" || $area == "ALL") && $sources == "GW1_U17") {
                                    $area_name = 'Grasso Section';
                                    $sources_name = 'GR-3';
                                  }
                                  if (($area == "GR" || $area == "ALL") && $sources == "GW1_U16") {
                                    $area_name = 'Grasso Section';
                                    $sources_name = 'GR-4';
                                  }
                                  if (($area == "GR" || $area == "ALL") && $sources == "GW1_U15") {
                                    $area_name = 'Grasso Section';
                                    $sources_name = 'GR-5';
                                  }
                                  if (($area == "GR" || $area == "ALL") && $sources == "GW1_U20") {
                                    $area_name = 'Grasso Section';
                                    $sources_name = 'GR-6';
                                  }
                                  if (($area == "GR" || $area == "ALL") && $sources == "GW1_U21") {
                                    $area_name = 'Grasso Section';
                                    $sources_name = 'GR-7';
                                  }
                                  if (($area == "GR" || $area == "ALL") && $sources == "GW1_U14") {
                                    $area_name = 'Grasso Section';
                                    $sources_name = 'GR-8';
                                  } elseif (($area == "ECR" || $area == "ALL") && $sources == "GW3_U3") {
                                    $area_name = 'ECR Section';
                                    $sources_name = 'Syrup Room L5';
                                  }
                                  if (($area == "ECR" || $area == "ALL") && $sources == "GW3_U4") {
                                    $area_name = 'ECR Section';
                                    $sources_name = 'Sugar Dissolving No.2';
                                  }
                                  if (($area == "ECR" || $area == "ALL") && $sources == "GW3_U5") {
                                    $area_name = 'ECR Section';
                                    $sources_name = 'Oven L5';
                                  }
                                  if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U2") {
                                    $area_name = 'ECR Section';
                                    $sources_name = 'Syrup Room L4';
                                  }
                                  if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U3") {
                                    $area_name = 'ECR Section';
                                    $sources_name = 'Juice Room L3';
                                  }
                                  if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U4") {
                                    $area_name = 'ECR Section';
                                    $sources_name = 'New Boiler';
                                  }
                                  if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U5") {
                                    $area_name = 'ECR Section';
                                    $sources_name = 'Sugar Dissolving No.1';
                                  } elseif (($area == "LI" || $area == "ALL") && $sources == "GW1_U6") {
                                    $area_name = 'Lines Section';
                                    $sources_name = 'Line-1';
                                  }
                                  if (($area == "LI" || $area == "ALL") && $sources == "GW1_U7") {
                                    $area_name = 'Lines Section';
                                    $sources_name = 'Line-2';
                                  }
                                  if (($area == "LI" || $area == "ALL") && $sources == "GW1_U23") {
                                    $area_name = 'Lines Section';
                                    $sources_name = 'Line-3';
                                  }
                                  // if (($area == "LI" || $area == "ALL" )&& $sources == "GW2_U2") {
                                  //   $area_name = 'Lines Section';
                                  //   $sources_name = 'Line-4';
                                  // }
                                  if (($area == "LI" || $area == "ALL") && $sources == "GW3_U2") {
                                    $area_name = 'Lines Section';
                                    $sources_name = 'Line-5';
                                  }
                                  if (($area == "LI" || $area == "ALL") && $sources == "GW1_U24") {
                                    $area_name = 'Lines Section';
                                    $sources_name = 'Line-6';
                                  }
                                  if (($area == "LI" || $area == "ALL") && $sources == "GW3_U6") {
                                    $area_name = 'Lines Section';
                                    $sources_name = 'Line-8';
                                  } elseif (($area == "LP" || $area == "ALL") && $sources == "GW2_U12") {
                                    $area_name = 'LPAC Section';
                                    $sources_name = 'GA90-VSD(Old)';
                                  }
                                  if (($area == "LP" || $area == "ALL") && $sources == "GW2_U11") {
                                    $area_name = 'LPAC Section';
                                    $sources_name = 'GA90-Old';
                                  }
                                  if (($area == "LP" || $area == "ALL") && $sources == "GW2_U14") {
                                    $area_name = 'LPAC Section';
                                    $sources_name = 'GA90-VSD(New)';
                                  }
                                  if (($area == "LP" || $area == "ALL") && $sources == "GW2_U13") {
                                    $area_name = 'LPAC Section';
                                    $sources_name = 'GA90-New';
                                  } elseif (($area == "HP" || $area == "ALL") && $sources == "GW2_U3") {
                                    $area_name = 'HPAC Section';
                                    $sources_name = 'HPAC-1425';
                                  }
                                  if (($area == "HP" || $area == "ALL") && $sources == "GW2_U4") {
                                    $area_name = 'HPAC Section';
                                    $sources_name = 'HPAC-2189';
                                  }
                                  if (($area == "HP" || $area == "ALL") && $sources == "GW2_U2") {
                                    $area_name = 'HPAC Section';
                                    $sources_name = 'HPAC4-HP1000';
                                  }
                                ?>
                                  <tr align="center">
                                    <td style="text-align: center;"><?php echo $i; ?></td>
                                    <td style="text-align: center;"><?php echo $sources_name; ?></td>
                                    <?php if ($kwh > 0) { ?>
                                      <td style="text-align: center;"><?php echo number_format($kwh, 2); ?></td>
                                    <?php } else { ?>
                                      <td>0</td>
                                    <?php } ?>
                                    <td><?php echo number_format($kwh * $_POST['unit'], 2); ?></td>
                                  </tr>
                                <?php

                                  $i++;
                                  $total_cost += $kwh * $_POST['unit'];
                                }
                                ?>
                              </tbody>
                            </table>
                          </div>
                        </div>
                      </div>
                      <div class="row">
                        <div class="col-sm-12">
                          <table class="table table-responsive invoice-table invoice-total" id="total">
                            <tbody>
                              <tr>
                                <th>Sub Total :</th>
                                <td><?php echo number_format($total_cost,2); ?></td>
                              </tr>
                              <!-- <tr>
                                        <th>VAT Included in Total :</th>
                                        <td align="center"></td>
                                      </tr> -->
                              <tr class="text-info">
                                <td>
                                  <hr>
                                  <h5 class="text-primary">Energy Cost Total (Rs.) :</h5>
                                </td>
                                <td>
                                  <hr>
                                  <h5 class="text-primary"><?php echo number_format($total_cost,2); ?></h5>
                                </td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
                      </div>
                    </div>
                  </div>
                  <!-- Invoice card end -->
                </div>
              </div>
              <!-- Container ends -->
            </div>
          </div>
        </div>
      </div>
      <div id="styleSelector">
      </div>
    </div>
  </div>
  </div>
  </div>
  </div>
  </div>
  </div>
  <?php include('includes/footer.php'); ?>
  <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
<script type="text/javascript">
function ExportToExcel(type, fn, dl) {
       var elt = document.getElementById('testTable');
       var wb = XLSX.utils.table_to_book(elt, { sheet: "sheet1" });
       return dl ?
         XLSX.write(wb, { bookType: type, bookSST: true, type: 'base64' }):
         XLSX.writeFile(wb, fn || ('Energy_Usage.' + (type || 'xlsx')));
    }
</script>
  <script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/3.3.6/js/bootstrap.min.js"></script>
  <!-- <script type="text/javascript">
    function fnExcelReport() {
      var tab_text = "<table border='2px'><tr bgcolor='#87AFC6' style='height: 75px; text-align:center; width: 250px'>";
      var textRange;
      var j = 0;
      tab = document.getElementById('testTable'); // id of table

      for (j = 0; j < tab.rows.length; j++) {
        tab_text = tab_text + tab.rows[j].innerHTML + "</tr>";
        //tab_text=tab_text+"</tr>";
      }
      tab_text = tab_text + "</table>";
      tab_text = tab_text.replace(/<A[^>]*>|<\/A>/g, ""); //remove if u want links in your table
      tab_text = tab_text.replace(/<img[^>]*>/gi, ""); // remove if u want images in your table
      tab_text = tab_text.replace(/<input[^>]*>|<\/input>/gi, ""); // reomves input params

      var ua = window.navigator.userAgent;
      var msie = ua.indexOf("MSIE ");

      if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) // If Internet Explorer
      {
        txtArea1.document.open("txt/html", "replace");
        txtArea1.document.write(tab_text);
        txtArea1.document.close();
        txtArea1.focus();
        sa = txtArea1.document.execCommand("SaveAs", true, "Energy_Cost_Report.xls");
      } else //other browser not tested on IE 11
        try {
          var blob = new Blob([tab_text], {
            type: "application/vnd.ms-excel"
          });
          window.URL = window.URL || window.webkitURL;
          link = window.URL.createObjectURL(blob);
          a = document.createElement("a");
          if (document.getElementById("caption") != null) {
            a.download = document.getElementById("caption").innerText;
          } else {
            a.download = 'Energy_Cost_Report';
          }

          a.href = link;

          document.body.appendChild(a);

          a.click();

          document.body.removeChild(a);
        } catch (e) {}
      return false;

      // return (sa);
    }
  </script> -->
  <script type="text/javascript">
    var headerImgData;
    headerImgData = 'data:image/png;base64,';

    function generate() {
      var doc = new jsPDF('p', 'pt', 'a4', );


      // header of pdf 
      var header = function(data) {
        doc.setFontSize(18);
        doc.setTextColor(40);
        doc.setFontStyle('normal'); //margin,width , height  
        doc.addImage(headerImgData, 'JPEG', data.settings.margin.left, 20, 70, 80);
        doc.setTextColor(100);
        doc.text(570, 65, 'Naubahar Bottling Company', 'right');


        doc.setDrawColor(0, 0, 0);
        doc.line(15, 15, 595 - 15, 15);



        doc.line(15, 15, 15, 120 - 15);


        doc.setLineWidth(2);
        doc.line(15, 120 - 15, 595 - 15, 120 - 15);


        doc.setLineWidth(1);
        doc.line(595 - 15, 120 - 15, 595 - 15, 15);

        doc.line(15, 150 - 15, 595 - 15, 150 - 15);


        // get date 
        var today = new Date();
        var dd = today.getDate();

        var mm = today.getMonth() + 1;
        var yyyy = today.getFullYear();

        today1 = dd + '/' + mm + '/' + yyyy;


        // get time in 12 h format 
        var today = new Date();
        var hours = today.getHours();
        var minutes = today.getMinutes();
        var ampm = hours >= 12 ? 'PM' : 'AM';
        hours = hours % 12;
        hours = hours ? hours : 12; // the hour '0' should be '12'
        minutes = minutes < 10 ? '0' + minutes : minutes;
        strTime = hours + ':' + minutes + ' ' + ampm;
        doc.setFontSize(10);
        doc.setFontStyle('normal');
        doc.text(strTime, 530, 125);
        doc.text("Reported Date:  " + today1, 405, 125);



      };




      // page content headings design 
      doc.setFillColor(0, 122, 239);
      doc.rect(20, 150, 560, 20, 'F');
      doc.setFontSize(16);
      doc.setTextColor(255, 255, 255)
      doc.text(150, 165, 'Energy Cost Report', 'center');



      // page content borders 
      doc.setLineWidth(1);
      doc.setDrawColor(0, 0, 0);
      //left border
      doc.line(15, 780, 15, 120 - 15);
      //right border
      doc.line(595 - 15, 780, 595 - 15, 120 - 15);
      //bottom border
      doc.line(15, 780, 595 - 15, 780);













      var footer = function(data) {

        // fonts of footer texts
        doc.setFontSize(10);
        doc.setFontStyle('normal');

        // single line specs for footer border
        doc.setLineWidth(3);
        doc.setDrawColor(0, 0, 0);
        doc.line(15, 800, 595 - 15, 800);

        // get page numbers 
        const pageCount = doc.internal.getNumberOfPages();

        // For each page, print the page number and the total pages
        for (var i = 1; i <= pageCount; i++) {
          // Go to page i
          doc.setPage(i);
          //Print Page 1 of 4 for example
          doc.text('Page ' + String(i) + ' of ' + String(pageCount), 530, 820);
        } // get date 
        var today = new Date();
        var dd = today.getDate();

        var mm = today.getMonth() + 1;
        var yyyy = today.getFullYear();

        today1 = dd + '/' + mm + '/' + yyyy;

        // get time in 12 h format 
        var hours = today.getHours();
        var minutes = today.getMinutes();
        var ampm = hours >= 12 ? 'PM' : 'AM';
        hours = hours % 12;
        hours = hours ? hours : 12; // the hour '0' should be '12'
        minutes = minutes < 10 ? '0' + minutes : minutes;
        strTime = hours + ':' + minutes + ' ' + ampm;
        doc.text("Generated on: " + today1, 15, 820);
        doc.text(strTime, 140, 820);


      }

      var options = { // variable for autotable for table specs 

        beforePageContent: header,

        afterPageContent: footer,

        margin: {
          top: 350,
          left: 30,
          right: 30,
          bottom: 45
        },
        //drawHeaderCell: function (cell, data) {        >>>>>>>>>>>controling individual column
        //  console.log(cell.text[0]);

        //  if (cell.text[0] ==='No.') {//paint.Name header red
        // cell.styles.fontSize= 12;
        //   cell.styles.textColor = [255,255,255];
        //   cell.styles.fillColor = "#43a089";
        //   console.log('pop');
        // } else {
        //     cell.styles.textColor = 255;
        //     cell.styles.fontSize = 10;

        // }
        //},


        startY: false,
        theme: 'grid',
        tableWidth: 'auto',
        columnWidth: 'wrap',
        showHeader: 'everyPage',
        tableLineColor: 200,
        tableLineWidth: 2,
        columnStyles: {
          0: {
            columnWidth: 'auto',
            halign: 'center'
          },
          1: {
            columnWidth: 'auto',
            halign: 'center'
          },
          2: {
            columnWidth: 'auto',
            halign: 'center'
          },
          3: {
            columnWidth: 'auto',
            halign: 'center'
          },
          4: {
            columnWidth: 'auto',
            halign: 'center'
          },


        },
        headerStyles: {
          //   theme:'plain',
          halign: 'center',
          fontSize: 12,
          textColor: [0, 0, 0],
          fillColor: [62, 161, 255],
        },
        footerStyles: {
          theme: 'plain',
          halign: 'center'
        },
        drawRow: function(cell, data) {


          if (odd_no < 17) {
            //console.log(data.table.rows[odd_no]);
            if (data.row.index === odd_no) {

              data.row.cells[0].styles.fillColor = [180, 218, 255];
              data.row.cells[1].styles.fillColor = [180, 218, 255];
              data.row.cells[2].styles.fillColor = [180, 218, 255];
              data.row.cells[3].styles.fillColor = [180, 218, 255];
              odd_no = odd_no + 2;
            }


          }


          //let a = cell.textPos;
          //console.log (a.x);
          // if (cell.textPos.length ===0) {console.log('opp')}

        },
        //willDrawCell: function (data) {
        //console.log(data.table.body);
        //var rows = data.table.body;
        //if (data.row.index === rows.length - 1) {
        //   doc.setFillColor(239, 154, 154);
        //}
        //},
        styles: {

          overflow: 'linebreak',
          columnWidth: 'wrap',
          font: 'arial',
          fontSize: 9,
          textColor: [0, 0, 0],
          //fillColor: [180,218,255],
          cellPadding: 8,
          overflowColumns: 'linebreak',
          halign: 'right'
        },
      };
      let odd_no = 0;
      var res1 = doc.autoTableHtmlToJson(document.getElementById("testTable"));
      doc.autoTable(res1.columns, res1.rows, options);
      var options1 = { // variable for autotable for table specs 
        beforePageContent: header,
        afterPageContent: footer,
        margin: {
          top: 600,
          left: 30,
          right: 30,
          bottom: 45
        },
        startY: false,
        theme: 'grid',
        tableWidth: 'auto',
        columnWidth: 'wrap',
        showHeader: 'everyPage',
        tableLineColor: 200,
        tableLineWidth: 2,
        columnStyles: {
          0: {
            columnWidth: 'auto',
            halign: 'center'
          },
          1: {
            columnWidth: 'auto',
            halign: 'center'
          },
          2: {
            columnWidth: 'auto',
            halign: 'center'
          },
          3: {
            columnWidth: 'auto',
            halign: 'center'
          },
          4: {
            columnWidth: 'auto',
            halign: 'center'
          },


        },
        headerStyles: {
          //   theme:'plain',
          halign: 'center',
          fontSize: 12,
          textColor: [0, 0, 0],
          fillColor: [62, 161, 255],
        },
        footerStyles: {
          theme: 'plain',
          halign: 'center'
        },
        drawRow: function(cell, data) {


          if (odd_no < 17) {
            //console.log(data.table.rows[odd_no]);
            if (data.row.index === odd_no) {

              data.row.cells[0].styles.fillColor = [180, 218, 255];
              data.row.cells[1].styles.fillColor = [180, 218, 255];
              data.row.cells[2].styles.fillColor = [180, 218, 255];
              data.row.cells[3].styles.fillColor = [180, 218, 255];
              odd_no = odd_no + 2;
            }
          }
        },
        styles: {
          overflow: 'linebreak',
          columnWidth: 'wrap',
          font: 'arial',
          fontSize: 9,
          textColor: [0, 0, 0],
          //fillColor: [180,218,255],
          cellPadding: 8,
          overflowColumns: 'linebreak',
          halign: 'right'
        },
      };
      var res2 = doc.autoTableHtmlToJson(document.getElementById("total"));
      doc.autoTable(res2.columns, res2.rows, options1);



      var options2 = { // variable for autotable for table specs 
        // beforePageContent: header,
        // afterPageContent:footer,
        margin: {
          top: 200,
          left: 200,
          right: 30,
          bottom: 45
        },
        startY: false,
        theme: 'grid',
        tableWidth: 'auto',
        columnWidth: 'wrap',
        showHeader: 'everyPage',
        tableLineColor: 200,
        tableLineWidth: 2,
        columnStyles: {
          0: {
            columnWidth: 'auto',
            halign: 'center'
          },
          1: {
            columnWidth: 'auto',
            halign: 'center'
          },
          2: {
            columnWidth: 'auto',
            halign: 'center'
          },
          3: {
            columnWidth: 'auto',
            halign: 'center'
          },
          4: {
            columnWidth: 'auto',
            halign: 'center'
          },


        },
        headerStyles: {
          //   theme:'plain',
          halign: 'center',
          fontSize: 12,
          textColor: [0, 0, 0],
          fillColor: [62, 161, 255],
        },
        footerStyles: {
          theme: 'plain',
          halign: 'center'
        },
        drawRow: function(cell, data) {


          if (odd_no < 17) {
            //console.log(data.table.rows[odd_no]);
            if (data.row.index === odd_no) {

              data.row.cells[0].styles.fillColor = [180, 218, 255];
              data.row.cells[1].styles.fillColor = [180, 218, 255];
              // data.row.cells[2].styles.fillColor = [180,218,255];
              // data.row.cells[3].styles.fillColor = [180,218,255];
              odd_no = odd_no + 2;
            }
          }
        },
        styles: {
          overflow: 'linebreak',
          columnWidth: 'wrap',
          font: 'arial',
          fontSize: 9,
          textColor: [0, 0, 0],
          //fillColor: [180,218,255],
          cellPadding: 8,
          overflowColumns: 'linebreak',
          halign: 'right'
        },
      };
      var res3 = doc.autoTableHtmlToJson(document.getElementById("test"));
      doc.autoTable(res3.columns, res3.rows, options2);

      doc.save("Energy_Cost_Report" + today1 + ".pdf");
    }
  </script>
  <script type="text/javascript" src="https://cdn.rawgit.com/simonbengtsson/jsPDF/requirejs-fix-dist/dist/jspdf.debug.js"></script>
  <script type="text/javascript" src="https://unpkg.com/jspdf-autotable@2.3.2"></script>
</body>

</html>