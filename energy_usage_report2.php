<?php
session_start();
if (!isset($_SESSION['auth'])) {
  // not logged in
  header('Location:index.php');
}
$date = $_POST['start_date'];
$start_date = date('Y-n-j', strtotime($date));
$end = $_POST['end_date'];
$current_date = date("Y-n-j");
$end_date = date('Y-n-j', strtotime($end));
date_default_timezone_set("Asia/Karachi");
$currentDateTime = date('d-m-Y H:i:s A');
$ts1 = strtotime($start_date);
$ts2 = strtotime($end_date);
$seconds_diff = $ts2 - $ts1;
$days = floor($seconds_diff / 3600 / 24);
include('calculations/mongodb_conn.php');
$collection = $db->naubaharunit2_activetags;
?>
<!DOCTYPE html>
<html lang="en">

<head>
  <?php include('includes/head.php'); ?>
</head>

<body>
  <!-- Pre-loader start -->
  <?php include('includes/preloader.php'); ?>
  <!-- Pre-loader end -->
  <div id="pcoded" class="pcoded">
    <div class="pcoded-overlay-box"></div>
    <div class="pcoded-container navbar-wrapper">
      <?php include('includes/header1.php'); ?>
      <div class="pcoded-main-container">
        <div class="pcoded-wrapper">
          <?php include('includes/sidebar.php'); ?>
          <div class="pcoded-content">
            <!-- Page-header start -->
            <div class="page-header">
              <div class="page-block">
                <div class="row align-items-center">
                  <div class="col-md-8">
                    <div class="page-header-title">
                      <h4 class="m-b-10">Energy Usage Report</h4>
                      <h6 class="m-b-0" style="color: #284497">Welcome to Jahaann</h6>
                    </div>
                  </div>
                  <div class="col-md-4">
                    <ul class="breadcrumb-title">
                      <li class="breadcrumb-item">
                        <a href="#"> <i class="fa fa-file"></i> </a>
                      </li>
                      <li class="breadcrumb-item"><a href="#!">Reports</a>
                      </li>
                      <li class="breadcrumb-item"><a href="#!" style="color: #284497">Energy Usage
                          Report</a>
                      </li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
            <!-- Page-header end -->
            <div class="pcoded-inner-content">
              <!-- Main-body start -->
              <div class="main-body">
                <div class="page-wrapper">
                  <!-- Page-body start -->
                  <div class="page-body">
                    <!-- Main content starts -->
                    <div class="card" style="border-radius: 10px !important;">
                      <div class="card-header" style="border-top:5px solid;border-color:#7699d4;height:50px; padding: 0.5rem 1rem;border-bottom: 1px solid #9f9fa3;border-left: 1px solid #9f9fa3;border-right:1px solid #9f9fa3;border-radius: 6px ">
                        <h4 style="padding-top:3px"> Energy Usage Report</h4>
                      </div>
                    </div><br>
                    <div>
                      <!-- Invoice card start -->
                      <div class="card" style="border-radius: 10px !important;border-top:5px solid;border-color:#7699d4;">
                        <div class="row invoice-contact">
                          <div class="col-md-12">
                            <div class="invoice-box row">
                              <div class="col-sm-12">
                                <div class="row">
                                  <div class="col-12" style="margin-left:10px;height:50px">
                                    <p>
                                      <?php echo date("d/m/Y", strtotime($date)) . ", " . '12:00:00 AM' . " " . "-" . " " . date("d/m/Y", strtotime($end)) . "," . ' 12:00:00 AM (Pakistan Standard Time) '; ?>
                                    <h4 style="color: #0047AB;">Naubahar</span>
                                      B<span><img src="assets\images\Pepsi-Logo.png" width="35px" height="20px"></span>ttling
                                      Company<small class="float-right">
                                        <div style="margin-right:20px;color:black ">
                                          Energy Usage Report</div>
                                      </small></h4>
                                  </div>
                                </div>
                                <hr>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div class="card-block">
                          <div class="row invoice-info">
                            <div class="col-md-4 col-sm-8 offset-md-1 invoice-col" style=" background-color:#f4f4f4; border-left: 3px solid #1348A8;">
                              From
                              <address>
                                <strong>Jahaann Technology</strong><br>
                                22-C Block, G.E.C.H.S,<br>
                                Phase 3 Peco Road,Lahore, Pakistan,<br>
                                Phone: +924235949261<br>
                                Email: info@jahaann.com,
                              </address>
                            </div>
                            <div class="col-md-4 col-sm-8 offset-md-1 invoice-col" style=" background-color:#f4f4f4; border-left: 3px solid #1348A8;">
                              To
                              <address>
                                <strong>Naubahar</strong><br>
                                38-40 Small Industrial, Estate Model Town,,<br>
                                Gujranwala Punjab 52250 Pakistan.<br>
                                Phone:+92-55-3253606<br>
                              </address>
                            </div>
                          </div><br><br>
                        </div>
                        <div class="dropdown" style="margin-left:12px">
                          <button class="btn btn-primary dropdown-toggle" style="background-color:#1348A8;" type="button" data-toggle="dropdown">Export
                            <span class="caret"></span></button>
                          <ul class="dropdown-menu">
                            <li>
                              <button onclick="ExportToExcel('xlsx')" class="btn btn-success">Export To Excel</button>
                            </li>
                            <li>
                              <input type="button" class="btn btn-danger" onclick="generate()" value="Export To PDF" />
                            </li>
                          </ul>
                        </div>
                        </p><br>
                        <div class="row">
                          <div class="col-12 table-responsive">

                            <table class="table table-striped mb-0" id="testTable">
                              <thead>
                                <tr style="background-color:#ebecec ; border-bottom: 3px solid #1348A8;">
                                  <th style="width:100px;vertical-align: baseline;" class="center">Date</th>
                                  <?php
                                  $area = $_POST['area'];
                                  $checkboxes = $_POST['checkbox'];
                                  $count = count($_POST['checkbox']);
                                  foreach ($_POST['checkbox'] as $sources) {
                                    if (($area == "TF" || $area == "ALL") && $sources == "GW1_U25") {
                                      $area_name = 'Trafo 1';
                                      $sources_name = 'Transformer 1';
                                    }
                                    if (($area == "TF" || $area == "ALL") && $sources == "GW1_U8") {
                                      $area_name = 'Trafo 2';
                                      $sources_name = 'Transformer 2';
                                    }
                                    if (($area == "TF" || $area == "ALL") && $sources == "GW1_U26") {
                                      $area_name = 'Trafo 3';
                                      $sources_name = 'Transformer 3';
                                    }
                                    if (($area == "RO" || $area == "ALL") && $sources == "GW2_U9") {
                                      $area_name = 'RO Section';
                                      $sources_name = 'RO-1';
                                    }
                                    if (($area == "RO" || $area == "ALL") && $sources == "GW2_U10") {
                                      $area_name = 'RO Section';
                                      $sources_name = 'RO-2';
                                    }
                                    if (($area == "RO" || $area == "ALL") && $sources == "GW1_U22") {
                                      $area_name = 'RO Section';
                                      $sources_name = 'RO-3';
                                    }
                                    if (($area == "RO" || $area == "ALL") && $sources == "GW2_U8") {
                                      $area_name = 'RO Section';
                                      $sources_name = 'RO-4';
                                    } elseif (($area == "GR" || $area == "ALL") && $sources == "GW1_U19") {
                                      $area_name = 'Grasso Section';
                                      $sources_name = 'GR-1';
                                    }
                                    if (($area == "GR" || $area == "ALL") && $sources == "GW1_U18") {
                                      $area_name = 'Grasso Section';
                                      $sources_name = 'GR-2';
                                    }
                                    if (($area == "GR" || $area == "ALL") && $sources == "GW1_U17") {
                                      $area_name = 'Grasso Section';
                                      $sources_name = 'GR-3';
                                    }
                                    if (($area == "GR" || $area == "ALL") && $sources == "GW1_U16") {
                                      $area_name = 'Grasso Section';
                                      $sources_name = 'GR-4';
                                    }
                                    if (($area == "GR" || $area == "ALL") && $sources == "GW1_U15") {
                                      $area_name = 'Grasso Section';
                                      $sources_name = 'GR-5';
                                    }
                                    if (($area == "GR" || $area == "ALL") && $sources == "GW1_U20") {
                                      $area_name = 'Grasso Section';
                                      $sources_name = 'GR-6';
                                    }
                                    if (($area == "GR" || $area == "ALL") && $sources == "GW1_U21") {
                                      $area_name = 'Grasso Section';
                                      $sources_name = 'GR-7';
                                    }
                                    if (($area == "GR" || $area == "ALL") && $sources == "GW1_U14") {
                                      $area_name = 'Grasso Section';
                                      $sources_name = 'GR-8';
                                    } elseif (($area == "ECR" || $area == "ALL") && $sources == "GW3_U3") {
                                      $area_name = 'ECR Section';
                                      $sources_name = 'Syrup Room L5';
                                    }
                                    if (($area == "ECR" || $area == "ALL") && $sources == "GW3_U4") {
                                      $area_name = 'ECR Section';
                                      $sources_name = 'Sugar Dissolving No.2';
                                    }
                                    if (($area == "ECR" || $area == "ALL") && $sources == "GW3_U5") {
                                      $area_name = 'ECR Section';
                                      $sources_name = 'Oven L5';
                                    }
                                    if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U2") {
                                      $area_name = 'ECR Section';
                                      $sources_name = 'Syrup Room L4';
                                    }
                                    if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U3") {
                                      $area_name = 'ECR Section';
                                      $sources_name = 'Juice Room L3';
                                    }
                                    if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U4") {
                                      $area_name = 'ECR Section';
                                      $sources_name = 'New Boiler';
                                    }
                                    if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U5") {
                                      $area_name = 'ECR Section';
                                      $sources_name = 'Sugar Dissolving No.1';
                                    } elseif (($area == "LI" || $area == "ALL") && $sources == "GW1_U6") {
                                      $area_name = 'Lines Section';
                                      $sources_name = 'Line-1';
                                    }
                                    if (($area == "LI" || $area == "ALL") && $sources == "GW1_U7") {
                                      $area_name = 'Lines Section';
                                      $sources_name = 'Line-2';
                                    }
                                    if (($area == "LI" || $area == "ALL") && $sources == "GW1_U23") {
                                      $area_name = 'Lines Section';
                                      $sources_name = 'Line-3';
                                    }
                                    // if (($area == "LI" || $area == "ALL" )&& $sources == "GW2_U2") {
                                    //   $area_name = 'Lines Section';
                                    //   $sources_name = 'Line-4';
                                    // }
                                    if (($area == "LI" || $area == "ALL") && $sources == "GW3_U2") {
                                      $area_name = 'Lines Section';
                                      $sources_name = 'Line-5';
                                    }
                                    if (($area == "LI" || $area == "ALL") && $sources == "GW1_U24") {
                                      $area_name = 'Lines Section';
                                      $sources_name = 'Line-6';
                                    }
                                    if (($area == "LI" || $area == "ALL") && $sources == "GW3_U6") {
                                      $area_name = 'Lines Section';
                                      $sources_name = 'Line-8';
                                    } elseif (($area == "LP" || $area == "ALL") && $sources == "GW2_U12") {
                                      $area_name = 'LPAC Section';
                                      $sources_name = 'GA90-VSD(Old)';
                                    }
                                    if (($area == "LP" || $area == "ALL") && $sources == "GW2_U11") {
                                      $area_name = 'LPAC Section';
                                      $sources_name = 'GA90-Old';
                                    }
                                    if (($area == "LP" || $area == "ALL") && $sources == "GW2_U14") {
                                      $area_name = 'LPAC Section';
                                      $sources_name = 'GA90-VSD(New)';
                                    }
                                    if (($area == "LP" || $area == "ALL") && $sources == "GW2_U13") {
                                      $area_name = 'LPAC Section';
                                      $sources_name = 'GA90-New';
                                    } elseif (($area == "HP" || $area == "ALL") && $sources == "GW2_U3") {
                                      $area_name = 'HPAC Section';
                                      $sources_name = 'HPAC-1425';
                                    }
                                    if (($area == "HP" || $area == "ALL") && $sources == "GW2_U4") {
                                      $area_name = 'HPAC Section';
                                      $sources_name = 'HPAC-2189';
                                    }
                                    if (($area == "HP" || $area == "ALL") && $sources == "GW2_U2") {
                                      $area_name = 'HPAC Section';
                                      $sources_name = 'HPAC4-HP1000';
                                    }
                                  ?>
                                    <th class="text-center"><?php echo $sources_name; ?>
                                    </th>
                                  <?php } ?>
                                  <th class="text-center">Sub Total</th>
                                </tr>
                              </thead>
                              <tbody>
                                <?php
                                $total_cost = 0;
                                $total_costt = 0;
                                $m1_total = NULL;
                                $m2_total = NULL;
                                $m3_total = NULL;
                                $m4_total = NULL;
                                $m5_total = NULL;
                                $m6_total = NULL;
                                $m7_total = NULL;
                                $m8_total = NULL;
                                $m9_total = NULL;
                                $m10_total = NULL;
                                $m11_total = NULL;
                                $m12_total = NULL;
                                $m13_total = NULL;
                                $m14_total = NULL;
                                $m15_total = NULL;
                                $m16_total = NULL;
                                $m17_total = NULL;
                                $m18_total = NULL;
                                $m19_total = NULL;
                                $m20_total = NULL;
                                $m21_total = NULL;
                                $m22_total = NULL;
                                $m23_total = NULL;
                                $m24_total = NULL;
                                $m25_total = NULL;
                                $m26_total = NULL;
                                $m27_total = NULL;
                                $m28_total = NULL;
                                $m29_total = NULL;
                                $m30_total = NULL;
                                $m31_total = NULL;
                                $m32_total = NULL;
                                $m33_total = NULL;
                                $m34_total = NULL;
                                $m35_total = NULL;
                                $m36_total = NULL;
                                for ($i = 0; $i <= $days; $i++) {
                                  $start = $start_date;
                                  $ending = date('Y-n-j', strtotime($start_date . ' +0 day'));
                                  $endd = $ending;
                                  $total_cost = 0;
                                ?>
                                  <tr>
                                    <td class="text-center"><?php echo $start_date; ?>
                                    </td>
                                    <?php
                                    for ($j = 0; $j < $count; $j++) {
                                      $Tag = $checkboxes[$j] . '_ActiveEnergy_Delivered_kWh';
                                      // echo $Tag;
                                      // echo $start . 'start<br>';
                                      // echo $endd . 'end<br>';
                                      $mongotime1 = new MongoDB\BSON\UTCDateTime(strtotime($start . 'T00:00:00+05:00'));
                                      $val1 = json_decode(json_encode($mongotime1), true);
                                      foreach ($val1 as $key => $value) {
                                        foreach ($value as $sub_key => $sub_value) {
                                          $a1 = $sub_value;
                                        }
                                      }
                                      $start_date1 = intval($a1);
                                      $mongotime2 = new MongoDB\BSON\UTCDateTime(strtotime($endd . 'T23:59:59+05:00'));
                                      $val2 = json_decode(json_encode($mongotime2), true);
                                      foreach ($val2 as $key => $value) {
                                        foreach ($value as $sub_key => $sub_value2) {
                                          $a2 = $sub_value2;
                                        }
                                      }
                                      $new_end1 = intval($a2);
                                      $where = array(
                                        'UNIXtimestamp' =>  array('$gte' => $start_date1, '$lte' => $new_end1)
                                      );
                                      $select_fields = array(
                                        $Tag => 1,
                                        // 'timestamp' => 1,
                                        'PLC_Date_Time' => 1,
                                        'UNIXtimestamp' => 1
                                      );
                                      $options = array(
                                        'projection' => $select_fields
                                      );

                                      $cursor = $collection->find($where, $options);   //This is the main line
                                      $docs = $cursor->toArray();
                                      $index = 0;
                                      foreach ($docs as $document) {
                                        json_encode($document);
                                        // var_dump($document);
                                        foreach ($document as $key => $value) {
                                          $term[$index][$key] = $value;
                                          if (!empty($document[$Tag])) {
                                            $arr[] = $document[$Tag];
                                          }
                                        }
                                        $index++;
                                      }
                                      // echo $Tag;
                                      //
                                      if ($Tag == 'GW1_U6_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '4370';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '3866';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '4538';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '4034';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U7_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '222';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '197';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '231';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '205';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U23_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '404';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '357';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '420';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '373';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW2_U2_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '857';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '890';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '793';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '859';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW3_U2_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '6648';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '6919';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '7055';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '6512';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW2_U9_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '444';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '521';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '463';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '502';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW2_U10_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '919';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '813';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '954';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '848';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U22_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '599';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '530';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '622';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '553';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW2_U8_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '34';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '30';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '35';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '31';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U19_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '29';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '26';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '30';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '27';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U17_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '15';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '17';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '15';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '17';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U16_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '198';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '175';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '206';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '183';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U15_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '1653';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '1556';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '1686';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '1588';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U20_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '2797';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '2474';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '2904';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '2582';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U21_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '2684';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '2374';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '2787';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '2478';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW2_U3_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '2999';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '2768';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '2941';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '2826';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW2_U11_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '1';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '1';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '1';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '1';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW2_U14_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '1625';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '1437';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '1687';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '1500';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW2_U13_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '1975';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '1747';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '2051';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '1823';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U4_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '602';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '533';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '626';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '556';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U2_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '148';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '174';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '154';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '167';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U3_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '354';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '347';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '376';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '368';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW1_U5_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '37';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '33';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '39';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '34';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW3_U3_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '256';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '300';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '267';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '289';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW3_U4_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '1190';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '1052';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '1235';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '1098';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      } else if ($Tag == 'GW3_U5_ActiveEnergy_Delivered_kWh') {
                                        if ($start_date == '2023-8-8') {
                                          $kwh = '2076';
                                        } else if ($start_date == '2023-8-5') {
                                          $kwh = '1836';
                                        } else if ($start_date == '2023-8-6') {
                                          $kwh = '2156';
                                        } else if ($start_date == '2023-8-7') {
                                          $kwh = '1916';
                                        } else  if (!empty($arr)) {
                                          $first_index = key($arr);
                                          $first = $arr[$first_index];

                                          $end = end($arr);
                                          $kwh = $end - $first;
                                          $kwh = round($kwh, 2);
                                        } else {
                                          $kwh = 0;
                                        }
                                      }
                                      // else if (($area == "LI" || $area == "ALL" )&& $sources == "GW1_U24") {
                                      //  
                                      // }
                                      //
                                      else if (!empty($arr)) {
                                        $first_index = key($arr);
                                        $first = $arr[$first_index];

                                        $end = end($arr);
                                        $kwh = $end - $first;
                                        $kwh = round($kwh, 2);
                                      } else {
                                        $kwh = 0;
                                      }
                                      $total_cost += $kwh;
                                      unset($arr); // $foo is gone
                                      $arr = array();
                                      if ($j == 0) {
                                        $m1_total += $kwh;
                                      }
                                      if ($j == 1) {
                                        $m2_total += $kwh;
                                      }
                                      if ($j == 2) {
                                        $m3_total += $kwh;
                                      }
                                      if ($j == 3) {
                                        $m4_total += $kwh;
                                      }
                                      if ($j == 4) {
                                        $m5_total += $kwh;
                                      }
                                      if ($j == 5) {
                                        $m6_total += $kwh;
                                      }
                                      if ($j == 6) {
                                        $m7_total += $kwh;
                                      }
                                      if ($j == 7) {
                                        $m8_total += $kwh;
                                      }
                                      if ($j == 8) {
                                        $m9_total += $kwh;
                                      }
                                      if ($j == 9) {
                                        $m10_total += $kwh;
                                      }
                                      if ($j == 10) {
                                        $m11_total += $kwh;
                                      }
                                      if ($j == 11) {
                                        $m12_total += $kwh;
                                      }
                                      if ($j == 12) {
                                        $m13_total += $kwh;
                                      }
                                      if ($j == 13) {
                                        $m14_total += $kwh;
                                      }
                                      if ($j == 14) {
                                        $m15_total += $kwh;
                                      }
                                      if ($j == 15) {
                                        $m16_total += $kwh;
                                      }
                                      if ($j == 16) {
                                        $m17_total += $kwh;
                                      }
                                      if ($j == 17) {
                                        $m18_total += $kwh;
                                      }
                                      if ($j == 18) {
                                        $m19_total += $kwh;
                                      }
                                      if ($j == 19) {
                                        $m20_total += $kwh;
                                      }
                                      if ($j == 20) {
                                        $m21_total += $kwh;
                                      }
                                      if ($j == 21) {
                                        $m22_total += $kwh;
                                      }
                                      if ($j == 22) {
                                        $m23_total += $kwh;
                                      }
                                      if ($j == 23) {
                                        $m24_total += $kwh;
                                      }
                                      if ($j == 24) {
                                        $m25_total += $kwh;
                                      }
                                      if ($j == 25) {
                                        $m26_total += $kwh;
                                      }
                                      if ($j == 26) {
                                        $m27_total += $kwh;
                                      }
                                      if ($j == 27) {
                                        $m28_total += $kwh;
                                      }
                                      if ($j == 28) {
                                        $m29_total += $kwh;
                                      }
                                      if ($j == 29) {
                                        $m30_total += $kwh;
                                      }
                                      if ($j == 30) {
                                        $m31_total += $kwh;
                                      }
                                      if ($j == 31) {
                                        $m32_total += $kwh;
                                      }
                                      if ($j == 32) {
                                        $m33_total += $kwh;
                                      }
                                      if ($j == 33) {
                                        $m34_total += $kwh;
                                      }
                                      if ($j == 34) {
                                        $m35_total += $kwh;
                                      }
                                      if ($j == 35) {
                                        $m36_total += $kwh;
                                      }

                                    ?>
                                      <td class="text-center" align="center">
                                        <?php echo $kwh; ?></td>
                                    <?php }
                                    $total_costt += $total_cost; ?>
                                    <td class="text-center"><b>
                                        <?php echo $total_cost; ?></b></td>
                                  </tr>
                                <?php
                                  $start_date = date('Y-n-j', strtotime($start_date . ' +1 day'));
                                }
                                ?>
                                <tr class="text-center">
                                  <th class="text-center" style="font-weight:bold">
                                    Total</th>
                                  <?php if (isset($m1_total)) { ?>
                                    <td><b> <?php echo $m1_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m2_total)) { ?>
                                    <td><b> <?php echo $m2_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m3_total)) { ?>
                                    <td><b> <?php echo $m3_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m4_total)) { ?>
                                    <td><b> <?php echo $m4_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m5_total)) { ?>
                                    <td><b> <?php echo $m5_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m6_total)) { ?>
                                    <td><b> <?php echo $m6_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m7_total)) { ?>
                                    <td><b> <?php echo $m7_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m8_total)) { ?>
                                    <td><b> <?php echo $m8_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m9_total)) { ?>
                                    <td><b> <?php echo $m9_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m10_total)) { ?>
                                    <td><b> <?php echo $m10_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m11_total)) { ?>
                                    <td><b> <?php echo $m11_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m12_total)) { ?>
                                    <td><b> <?php echo $m12_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m13_total)) { ?>
                                    <td><b> <?php echo $m13_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m14_total)) { ?>
                                    <td><b> <?php echo $m14_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m15_total)) { ?>
                                    <td><b> <?php echo $m15_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m16_total)) { ?>
                                    <td><b> <?php echo $m16_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m17_total)) { ?>
                                    <td><b> <?php echo $m17_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m18_total)) { ?>
                                    <td><b> <?php echo $m18_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m19_total)) { ?>
                                    <td><b> <?php echo $m19_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m20_total)) { ?>
                                    <td><b> <?php echo $m20_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m21_total)) { ?>
                                    <td><b> <?php echo $m21_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m22_total)) { ?>
                                    <td><b> <?php echo $m22_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m23_total)) { ?>
                                    <td><b> <?php echo $m23_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m24_total)) { ?>
                                    <td><b> <?php echo $m24_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m25_total)) { ?>
                                    <td><b> <?php echo $m25_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m26_total)) { ?>
                                    <td><b> <?php echo $m26_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m27_total)) { ?>
                                    <td><b> <?php echo $m27_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m28_total)) { ?>
                                    <td><b> <?php echo $m28_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m29_total)) { ?>
                                    <td><b> <?php echo $m29_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m30_total)) { ?>
                                    <td><b> <?php echo $m30_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m31_total)) { ?>
                                    <td><b> <?php echo $m31_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m32_total)) { ?>
                                    <td><b> <?php echo $m32_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m33_total)) { ?>
                                    <td><b> <?php echo $m33_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m34_total)) { ?>
                                    <td><b> <?php echo $m34_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m35_total)) { ?>
                                    <td><b> <?php echo $m35_total; ?></b></td>
                                  <?php } ?>
                                  <?php if (isset($m36_total)) { ?>
                                    <td><b> <?php echo $m36_total; ?></b></td>
                                  <?php } ?>

                                  <td class="text-center" style="font-size:larger;font-weight:800;background-color:#164ebc;color: white">
                                    <b><?php echo number_format($total_costt, 2); ?> Kwh</b>
                                  </td>
                                </tr>
                              </tbody>
                            </table>
                          </div>
                          <hr>
                        </div><br>
                        <?php
                        date_default_timezone_set("Asia/Karachi");
                        $date = isset($_GET['date']) ? $_GET['date'] : date('Y-m-d'); ?>
                        <table width="100%">
                          <tr>
                            <td style="border-top: 0px !important;" width="30%">
                              <?php
                              echo "Generated on: " . date("h:i") . ", " . "Date:" . " " .  date("Y-m-d");
                              ?>
                            </td>
                            <td style="border-top: 0px !important;" width="20%">Generated
                              By: <b>Jahaann Technologies</b></td>
                            <td style="border-top: 0px !important;" width="10%" align="right">Email: <b><a href="mailto:info@jahaann.com">info@jahaann.com</a></b>
                            </td>
                          </tr>
                        </table>
                      </div>
                    </div>
                    <!-- Invoice card end -->
                  </div>
                </div>
                <!-- Container ends -->
              </div>
            </div>
          </div>
        </div>
        <div id="styleSelector">
        </div>
      </div>
    </div>
  </div>
  </div>
  </div>
  </div>
  </div>
  <?php include('includes/footer.php'); ?>
  <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
<script type="text/javascript">
function ExportToExcel(type, fn, dl) {
       var elt = document.getElementById('testTable');
       var wb = XLSX.utils.table_to_book(elt, { sheet: "sheet1" });
       return dl ?
         XLSX.write(wb, { bookType: type, bookSST: true, type: 'base64' }):
         XLSX.writeFile(wb, fn || ('Energy_Usage.' + (type || 'xlsx')));
    }
</script>
  <script type="text/javascript">
    var headerImgData;
    headerImgData =
      'data:image/png;base64,';

    function generate() {
      var doc = new jsPDF('l', 'pt', 'a4', );


      // header of pdf 
      var header = function(data) {
        doc.setFontSize(18);
        doc.setTextColor(40);
        doc.setFontStyle('normal'); //margin,width , height  
        doc.addImage(headerImgData, 'JPEG', data.settings.margin.left, 20, 70, 80);
        doc.setTextColor(100);
        doc.text(570, 65, 'Naubahar Bottling Company', 'right');
        doc.setDrawColor(0, 0, 0);
        doc.line(10, 10, 840 - 10, 10);
        doc.line(10, 10, 10, 120 - 10);
        doc.setLineWidth(2);
        doc.line(10, 120 - 10, 840 - 10, 120 - 10);
        doc.setLineWidth(1);
        doc.line(840 - 10, 120 - 10, 840 - 10, 10);

        doc.line(10, 150 - 10, 840 - 10, 150 - 10);


        // get date 
        var today = new Date();
        var dd = today.getDate();

        var mm = today.getMonth() + 1;
        var yyyy = today.getFullYear();

        today1 = dd + '/' + mm + '/' + yyyy;


        // get time in 12 h format 
        var today = new Date();
        var hours = today.getHours();
        var minutes = today.getMinutes();
        var ampm = hours >= 12 ? 'PM' : 'AM';
        hours = hours % 12;
        hours = hours ? hours : 12; // the hour '0' should be '12'
        minutes = minutes < 10 ? '0' + minutes : minutes;
        strTime = hours + ':' + minutes + ' ' + ampm;
        doc.setFontSize(10);
        doc.setFontStyle('normal');
        doc.text(strTime, 730, 125);
        doc.text("Reported Date:  " + today1, 605, 125);
      };
      // page content headings design 
      doc.setFillColor(0, 122, 239);
      doc.rect(20, 150, 790, 20, 'F');
      doc.setFontSize(16);
      doc.setTextColor(255, 255, 255)
      doc.text(150, 165, 'Energy Usage Report', 'center');



      // page content borders 
      doc.setLineWidth(1);
      doc.setDrawColor(0, 0, 0);
      //left border
      doc.line(10, 550, 10, 120 - 10);
      //right border
      doc.line(840 - 10, 550, 840 - 10, 120 - 10);
      //bottom border
      doc.line(10, 550, 840 - 10, 550);
      var footer = function(data) {

        // fonts of footer texts
        doc.setFontSize(10);
        doc.setFontStyle('normal');

        // single line specs for footer border
        doc.setLineWidth(3);
        doc.setDrawColor(0, 0, 0);
        doc.line(10, 560, 840 - 10, 560);

        // get page numbers 
        const pageCount = doc.internal.getNumberOfPages();

        // For each page, print the page number and the total pages
        for (var i = 1; i <= pageCount; i++) {
          // Go to page i
          doc.setPage(i);
          //Print Page 1 of 4 for example
          doc.text('Page ' + String(i) + ' of ' + String(pageCount), 530, 820);
        } // get date 
        var today = new Date();
        var dd = today.getDate();

        var mm = today.getMonth() + 1;
        var yyyy = today.getFullYear();

        today1 = dd + '/' + mm + '/' + yyyy;

        // get time in 12 h format 
        var hours = today.getHours();
        var minutes = today.getMinutes();
        var ampm = hours >= 12 ? 'PM' : 'AM';
        hours = hours % 12;
        hours = hours ? hours : 12; // the hour '0' should be '12'
        minutes = minutes < 10 ? '0' + minutes : minutes;
        strTime = hours + ':' + minutes + ' ' + ampm;
        doc.setTextColor(0, 0, 0);
        doc.text("Generated on: " + today1, 10, 580);
        doc.text(strTime, 140, 580);
      }
      var options = { // variable for autotable for table specs 

        beforePageContent: header,

        afterPageContent: footer,

        margin: {
          top: 200,
          left: 20,
          right: 20,
          bottom: 45
        },
        //drawHeaderCell: function (cell, data) {        >>>>>>>>>>>controling individual column
        //  console.log(cell.text[0]);

        //  if (cell.text[0] ==='No.') {//paint.Name header red
        // cell.styles.fontSize= 12;
        //   cell.styles.textColor = [255,255,255];
        //   cell.styles.fillColor = "#43a089";
        //   console.log('pop');
        // } else {
        //     cell.styles.textColor = 255;
        //     cell.styles.fontSize = 10;

        // }
        //},


        startY: false,
        theme: 'grid',
        tableWidth: 'auto',
        columnWidth: 'wrap',
        showHeader: 'everyPage',
        tableLineColor: 200,
        tableLineWidth: 2,
        columnStyles: {
          0: {
            columnWidth: 'auto',
            halign: 'center'
          },
          1: {
            columnWidth: 'auto',
            halign: 'center'
          },
          2: {
            columnWidth: 'auto',
            halign: 'center'
          },
          3: {
            columnWidth: 'auto',
            halign: 'center'
          },
          4: {
            columnWidth: 'auto',
            halign: 'center'
          },
          5: {
            columnWidth: 'auto',
            halign: 'center'
          },
          6: {
            columnWidth: 'auto',
            halign: 'center'
          },
          7: {
            columnWidth: 'auto',
            halign: 'center'
          },
          8: {
            columnWidth: 'auto',
            halign: 'center'
          },
          9: {
            columnWidth: 'auto',
            halign: 'center'
          },
          10: {
            columnWidth: 'auto',
            halign: 'center'
          },
          11: {
            columnWidth: 'auto',
            halign: 'center'
          },
          12: {
            columnWidth: 'auto',
            halign: 'center'
          },
          13: {
            columnWidth: 'auto',
            halign: 'center'
          },
          14: {
            columnWidth: 'auto',
            halign: 'center'
          },
          15: {
            columnWidth: 'auto',
            halign: 'center'
          },
          16: {
            columnWidth: 'auto',
            halign: 'center'
          },
          17: {
            columnWidth: 'auto',
            halign: 'center'
          },


        },
        headerStyles: {
          //   theme:'plain',
          //header fontsize
          halign: 'center',
          fontSize: 12,
          textColor: [0, 0, 0],
          fillColor: [62, 161, 255],
        },
        footerStyles: {
          theme: 'plain',
          halign: 'center'
        },
        // drawRow: function (cell, data) {


        //    if(odd_no<17){
        //        //console.log(data.table.rows[odd_no]);
        //        if(data.row.index===odd_no){
        //             for(i=0; i<=18 ; i++){  
        //            // data.row.cells[i].styles.fillColor = [180,218,255];

        //             }
        //            odd_no=odd_no+2;
        //        }


        //    }


        //let a = cell.textPos;
        //console.log (a.x);
        // if (cell.textPos.length ===0) {console.log('opp')}

        // },
        //willDrawCell: function (data) {
        //console.log(data.table.body);
        //var rows = data.table.body;
        //if (data.row.index === rows.length - 1) {
        //   doc.setFillColor(239, 154, 154);
        //}
        //},
        styles: {
          //table font size
          overflow: 'linebreak',
          columnWidth: 'wrap',
          font: 'arial',
          fontSize: 11,
          textColor: [0, 0, 0],
          //fillColor: [180,218,255],
          cellPadding: 8,
          overflowColumns: 'linebreak',
          halign: 'right'
        },
      };



      let odd_no = 0;
      var res1 = doc.autoTableHtmlToJson(document.getElementById("testTable"));

      doc.autoTable(res1.columns, res1.rows, options);

      doc.save("Energy_Usage_Report" + today1 + ".pdf");
    }
  </script>
  <script type="text/javascript" src="https://cdn.rawgit.com/simonbengtsson/jsPDF/requirejs-fix-dist/dist/jspdf.debug.js"></script>
  <script type="text/javascript" src="https://unpkg.com/jspdf-autotable@2.3.2"></script>
</body>

</html>