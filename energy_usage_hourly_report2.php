<?php
session_start();
if (!isset($_SESSION['auth'])) {
    // not logged in
    header('Location:index.php');
}
date_default_timezone_set("Asia/Karachi");

// MongoDB connection
include('calculations/mongodb_conn.php');
$collection = $db->naubaharunit2_activetags;

// Fetch selected tags (checkboxes) from the form submission
$checkboxes = $_POST['checkbox'];
$count = count($checkboxes);
$date = $_POST['start_date'];
$start_date = date('Y-n-j', strtotime($date));
$end = $_POST['end_date'];
$end_date = date('Y-n-j', strtotime($end . '+1 day'));
$end_date1 = date('Y-n-j', strtotime($end));
$current_date = date("Y-n-j");
// Define start and end dates (these can be replaced by actual inputs)
$start = $start_date;
$endd = $end_date; // Example for multiple days

// Convert start and end times to seconds (since your UNIXtimestamp is in seconds)
$mongotime1 = strtotime($start . 'T00:00:00+05:00');
$mongotime2 = strtotime($endd . 'T00:00:59+05:00');

// Prepare query and fields
$where = array(
    'UNIXtimestamp' => array('$gte' => $mongotime1, '$lte' => $mongotime2)
);

$options = array('projection' => array_merge(array(
    'PLC_Date_Time' => 1,
    'UNIXtimestamp' => 1
)));

// Dynamically add all tags to the select fields
for ($j = 0; $j < $count; $j++) {
    $Tag = $checkboxes[$j] . '_ActiveEnergy_Delivered_kWh';
    $options['projection'][$Tag] = 1; // Add each tag to projection
}

$cursor = $collection->find($where, $options);
$docs = $cursor->toArray();  // Fetch data

// Debugging step: Check if $docs has data
if (empty($docs)) {
    echo "No data found for the selected date range.";
    exit;
}

// Step 2: Initialize an array to store the hourly consumption for each tag
$hourlyConsumption = array();
foreach ($docs as $document) {
    // Remove the 'DT#' prefix and parse the date string
    $rawDateTime = str_replace('DT#', '', $document['PLC_Date_Time']);

    // Replace the last dash (-) between the date and time with a space
    $formattedDateTime = preg_replace('/(\d{4}-\d{2}-\d{2})-(\d{2}:\d{2}:\d{2})/', '$1 $2', $rawDateTime);

    // Create DateTime object
    $dateTime = new DateTime($formattedDateTime);
    $day = $dateTime->format('Y-m-d');  // Get the day (YYYY-MM-DD)
    $hour = $dateTime->format('H');  // Get the hour (HH)

    // For each tag, store the first value for each hour for each day
    for ($j = 0; $j < $count; $j++) {
        $Tag = $checkboxes[$j] . '_ActiveEnergy_Delivered_kWh';
        if (!isset($hourlyConsumption[$Tag][$day][$hour]) && isset($document[$Tag])) {
            $hourlyConsumption[$Tag][$day][$hour] = $document[$Tag];
        }
    }
}

// Step 3: Calculate hourly consumption for each tag and each day
foreach ($hourlyConsumption as $Tag => $daysData) {
    foreach ($daysData as $day => $hoursData) {
        $hours = array_keys($hoursData);
        for ($i = 0; $i < count($hours); $i++) {
            $currentHour = $hours[$i];

            // If we're processing the first hour (00:00), calculate the difference between 00:00 and 01:00
            if ($currentHour == '00') {
                if (isset($hoursData['01']) && isset($hoursData[$currentHour])) {
                    // Calculate consumption from 00:00 to 01:00
                    $hourlyTagConsumption[$Tag][$day][$currentHour] = $hoursData['01'] - $hoursData[$currentHour];
                } else {
                    // If data for 01:00 is missing, set consumption to 0
                    $hourlyTagConsumption[$Tag][$day][$currentHour] = 0;
                }
            } elseif ($i < count($hours) - 1) {
                // Normal case: calculate difference between consecutive hours (e.g., 01:00 to 02:00)
                $nextHour = $hours[$i + 1];
                if (isset($hoursData[$nextHour]) && isset($hoursData[$currentHour])) {
                    $hourlyTagConsumption[$Tag][$day][$currentHour] = $hoursData[$nextHour] - $hoursData[$currentHour];
                } else {
                    $hourlyTagConsumption[$Tag][$day][$currentHour] = 0;  // Default to 0 if data is missing
                }
            } else {
                // Special case: for the final hour of the day (23:00), calculate consumption up to the next day’s 00:00
                $nextDay = date('Y-m-d', strtotime($day . ' +1 day')); // Get the next day
                if (isset($hourlyConsumption[$Tag][$nextDay]['00'])) {
                    $hourlyTagConsumption[$Tag][$day][$currentHour] = $hourlyConsumption[$Tag][$nextDay]['00'] - $hoursData[$currentHour];
                } else {
                    // If next day’s data is not available, set consumption to 0 or apply another logic here
                    $hourlyTagConsumption[$Tag][$day][$currentHour] = 0;
                }
            }

            // Round the result for better readability
            $hourlyTagConsumption[$Tag][$day][$currentHour] = round($hourlyTagConsumption[$Tag][$day][$currentHour], 2);
        }
    }
}



// Step 4: Prepare for display in HTML (one table per day with multiple tags in columns)
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <?php include('includes/head.php'); ?>
</head>

<body>
    <!-- Pre-loader start -->
    <?php include('includes/preloader.php');
    ?>
    <div id="pcoded" class="pcoded">
        <div class="pcoded-overlay-box"></div>
        <div class="pcoded-container navbar-wrapper">
            <?php include('includes/header1.php'); ?>
            <div class="pcoded-main-container">
                <div class="pcoded-wrapper">
                    <?php include('includes/sidebar.php'); ?>
                    <div class="pcoded-content">
                        <!-- Page-header start -->
                        <div class="page-header">
                            <div class="page-block">
                                <div class="row align-items-center">
                                    <div class="col-md-8">
                                        <div class="page-header-title">
                                            <h4 class="m-b-10">Hourly Energy Usage Report</h4>
                                            <h6 class="m-b-0" style="color: #284497">Welcome to Jahaann</h6>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <ul class="breadcrumb-title">
                                            <li class="breadcrumb-item">
                                                <a href="#"> <i class="fa fa-file"></i> </a>
                                            </li>
                                            <li class="breadcrumb-item"><a href="#!">Reports</a>
                                            </li>
                                            <li class="breadcrumb-item"><a href="#!" style="color: #284497">Energy Usage
                                                    Report</a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Pre-loader end -->

                        <div class="pcoded-inner-content">
                            <!-- Main-body start -->
                            <div class="main-body">
                                <div class="page-wrapper">
                                    <!-- Page-body start -->
                                    <div class="page-body">
                                        <!-- Main content starts -->
                                        <div class="card" style="border-radius: 10px !important;">
                                            <div class="card-header" style="border-top:5px solid;border-color:#7699d4;height:50px; padding: 0.5rem 1rem;border-bottom: 1px solid #9f9fa3;border-left: 1px solid #9f9fa3;border-right:1px solid #9f9fa3;border-radius: 6px ">
                                                <h4 style="padding-top:3px"> Hourly Energy Usage Report</h4>
                                            </div>
                                        </div><br>
                                        <div>
                                            <!-- Invoice card start -->
                                            <div class="card" style="border-radius: 10px !important;border-top:5px solid;border-color:#7699d4;">
                                                <div class="row invoice-contact">
                                                    <div class="col-md-12">
                                                        <div class="invoice-box row">
                                                            <div class="col-sm-12">
                                                                <div class="row">
                                                                    <div class="col-12" style="margin-left:10px;height:50px">
                                                                        <p>
                                                                            <?php echo date("d/m/Y", strtotime($date)) . ", " . '12:00:00 AM' . " " . "-" . " " . date("d/m/Y", strtotime($end)) . "," . ' 12:00:00 AM (Pakistan Standard Time) '; ?>
                                                                        <h4 style="color: #0047AB;">Naubahar</span>
                                                                            B<span><img src="assets\images\Pepsi-Logo.png" width="35px" height="20px"></span>ttling
                                                                            Company<small class="float-right">
                                                                                <div style="margin-right:20px;color:black ">
                                                                                    Hourly Energy Usage Report</div>
                                                                            </small></h4>
                                                                    </div>
                                                                </div>
                                                                <hr>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="card-block">
                                                    <div class="row invoice-info">
                                                        <div class="col-md-4 col-sm-8 offset-md-1 invoice-col" style=" background-color:#f4f4f4; border-left: 3px solid #1348A8;">
                                                            From
                                                            <address>
                                                                <strong>Jahaann Technology</strong><br>
                                                                22-C Block, G.E.C.H.S,<br>
                                                                Phase 3 Peco Road,Lahore, Pakistan,<br>
                                                                Phone: +924235949261<br>
                                                                Email: info@jahaann.com,
                                                            </address>
                                                        </div>
                                                        <div class="col-md-4 col-sm-8 offset-md-1 invoice-col" style=" background-color:#f4f4f4; border-left: 3px solid #1348A8;">
                                                            To
                                                            <address>
                                                                <strong>Naubahar</strong><br>
                                                                38-40 Small Industrial, Estate Model Town,,<br>
                                                                Gujranwala Punjab 52250 Pakistan.<br>
                                                                Phone:+92-55-3253606<br>
                                                            </address>
                                                        </div>
                                                    </div><br><br>
                                                </div>
                                                <div class="dropdown" style="margin-left:12px">
                                                    <button class="btn btn-primary dropdown-toggle" style="background-color:#1348A8;" type="button" data-toggle="dropdown">Export
                                                        <span class="caret"></span></button>
                                                    <ul class="dropdown-menu">
                                                        <li>
                                                            <button onclick="ExportToExcel('xlsx')" class="btn btn-success">Export To Excel</button>
                                                        </li>
                                                        <li>
                                                            <input type="button" class="btn btn-danger" onclick="generate()" value="Export To PDF" />
                                                        </li>
                                                    </ul>
                                                </div>
                                                </p><br>

                                                <div class="row">
                                                    <div class="col-12 table-responsive">
                                                        <table class="table table-striped mb-0" id="testTable">
                                                            <thead>
                                                                <tr style="background-color:#ebecec ; border-bottom: 3px solid #1348A8;">
                                                                    <th style="width:150px;vertical-align: baseline;" class="center">Date & Hour</th>
                                                                    <?php
                                                                    // Display column headers for each selected checkbox (i.e., each tag)
                                                                    $area = $_POST['area'];
                                                                    $checkboxes = $_POST['checkbox'];
                                                                    $count = count($_POST['checkbox']);
                                                                    foreach ($_POST['checkbox'] as $sources) {
                                                                        if (($area == "TF" || $area == "ALL") && $sources == "GW1_U25") {
                                                                            $area_name = 'Trafo 1';
                                                                            $sources_name = 'Transformer 1';
                                                                        }
                                                                        if (($area == "TF" || $area == "ALL") && $sources == "GW1_U8") {
                                                                            $area_name = 'Trafo 2';
                                                                            $sources_name = 'Transformer 2';
                                                                        }
                                                                        if (($area == "TF" || $area == "ALL") && $sources == "GW1_U26") {
                                                                            $area_name = 'Trafo 3';
                                                                            $sources_name = 'Transformer 3';
                                                                        }
                                                                        if (($area == "RO" || $area == "ALL") && $sources == "GW2_U9") {
                                                                            $area_name = 'RO Section';
                                                                            $sources_name = 'RO-1';
                                                                        }
                                                                        if (($area == "RO" || $area == "ALL") && $sources == "GW2_U10") {
                                                                            $area_name = 'RO Section';
                                                                            $sources_name = 'RO-2';
                                                                        }
                                                                        if (($area == "RO" || $area == "ALL") && $sources == "GW1_U22") {
                                                                            $area_name = 'RO Section';
                                                                            $sources_name = 'RO-3';
                                                                        }
                                                                        if (($area == "RO" || $area == "ALL") && $sources == "GW2_U8") {
                                                                            $area_name = 'RO Section';
                                                                            $sources_name = 'RO-4';
                                                                        } elseif (($area == "GR" || $area == "ALL") && $sources == "GW1_U19") {
                                                                            $area_name = 'Grasso Section';
                                                                            $sources_name = 'GR-1';
                                                                        }
                                                                        if (($area == "GR" || $area == "ALL") && $sources == "GW1_U18") {
                                                                            $area_name = 'Grasso Section';
                                                                            $sources_name = 'GR-2';
                                                                        }
                                                                        if (($area == "GR" || $area == "ALL") && $sources == "GW1_U17") {
                                                                            $area_name = 'Grasso Section';
                                                                            $sources_name = 'GR-3';
                                                                        }
                                                                        if (($area == "GR" || $area == "ALL") && $sources == "GW1_U16") {
                                                                            $area_name = 'Grasso Section';
                                                                            $sources_name = 'GR-4';
                                                                        }
                                                                        if (($area == "GR" || $area == "ALL") && $sources == "GW1_U15") {
                                                                            $area_name = 'Grasso Section';
                                                                            $sources_name = 'GR-5';
                                                                        }
                                                                        if (($area == "GR" || $area == "ALL") && $sources == "GW1_U20") {
                                                                            $area_name = 'Grasso Section';
                                                                            $sources_name = 'GR-6';
                                                                        }
                                                                        if (($area == "GR" || $area == "ALL") && $sources == "GW1_U21") {
                                                                            $area_name = 'Grasso Section';
                                                                            $sources_name = 'GR-7';
                                                                        }
                                                                        if (($area == "GR" || $area == "ALL") && $sources == "GW1_U14") {
                                                                            $area_name = 'Grasso Section';
                                                                            $sources_name = 'GR-8';
                                                                        } elseif (($area == "ECR" || $area == "ALL") && $sources == "GW3_U3") {
                                                                            $area_name = 'ECR Section';
                                                                            $sources_name = 'Syrup Room L5';
                                                                        }
                                                                        if (($area == "ECR" || $area == "ALL") && $sources == "GW3_U4") {
                                                                            $area_name = 'ECR Section';
                                                                            $sources_name = 'Sugar Dissolving No.2';
                                                                        }
                                                                        if (($area == "ECR" || $area == "ALL") && $sources == "GW3_U5") {
                                                                            $area_name = 'ECR Section';
                                                                            $sources_name = 'Oven L5';
                                                                        }
                                                                        if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U2") {
                                                                            $area_name = 'ECR Section';
                                                                            $sources_name = 'Syrup Room L4';
                                                                        }
                                                                        if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U3") {
                                                                            $area_name = 'ECR Section';
                                                                            $sources_name = 'Juice Room L3';
                                                                        }
                                                                        if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U4") {
                                                                            $area_name = 'ECR Section';
                                                                            $sources_name = 'New Boiler';
                                                                        }
                                                                        if (($area == "ECR" || $area == "ALL") && $sources == "GW1_U5") {
                                                                            $area_name = 'ECR Section';
                                                                            $sources_name = 'Sugar Dissolving No.1';
                                                                        } elseif (($area == "LI" || $area == "ALL") && $sources == "GW1_U6") {
                                                                            $area_name = 'Lines Section';
                                                                            $sources_name = 'Line-1';
                                                                        }
                                                                        if (($area == "LI" || $area == "ALL") && $sources == "GW1_U7") {
                                                                            $area_name = 'Lines Section';
                                                                            $sources_name = 'Line-2';
                                                                        }
                                                                        if (($area == "LI" || $area == "ALL") && $sources == "GW1_U23") {
                                                                            $area_name = 'Lines Section';
                                                                            $sources_name = 'Line-3';
                                                                        }
                                                                        // if (($area == "LI" || $area == "ALL" )&& $sources == "GW2_U2") {
                                                                        //   $area_name = 'Lines Section';
                                                                        //   $sources_name = 'Line-4';
                                                                        // }
                                                                        if (($area == "LI" || $area == "ALL") && $sources == "GW3_U2") {
                                                                            $area_name = 'Lines Section';
                                                                            $sources_name = 'Line-5';
                                                                        }
                                                                        if (($area == "LI" || $area == "ALL") && $sources == "GW1_U24") {
                                                                            $area_name = 'Lines Section';
                                                                            $sources_name = 'Line-6';
                                                                        }
                                                                        if (($area == "LI" || $area == "ALL") && $sources == "GW3_U6") {
                                                                            $area_name = 'Lines Section';
                                                                            $sources_name = 'Line-8';
                                                                        } elseif (($area == "LP" || $area == "ALL") && $sources == "GW2_U12") {
                                                                            $area_name = 'LPAC Section';
                                                                            $sources_name = 'GA90-VSD(Old)';
                                                                        }
                                                                        if (($area == "LP" || $area == "ALL") && $sources == "GW2_U11") {
                                                                            $area_name = 'LPAC Section';
                                                                            $sources_name = 'GA90-Old';
                                                                        }
                                                                        if (($area == "LP" || $area == "ALL") && $sources == "GW2_U14") {
                                                                            $area_name = 'LPAC Section';
                                                                            $sources_name = 'GA90-VSD(New)';
                                                                        }
                                                                        if (($area == "LP" || $area == "ALL") && $sources == "GW2_U13") {
                                                                            $area_name = 'LPAC Section';
                                                                            $sources_name = 'GA90-New';
                                                                        } elseif (($area == "HP" || $area == "ALL") && $sources == "GW2_U3") {
                                                                            $area_name = 'HPAC Section';
                                                                            $sources_name = 'HPAC-1425';
                                                                        }
                                                                        if (($area == "HP" || $area == "ALL") && $sources == "GW2_U4") {
                                                                            $area_name = 'HPAC Section';
                                                                            $sources_name = 'HPAC-2189';
                                                                        }
                                                                        if (($area == "HP" || $area == "ALL") && $sources == "GW2_U2") {
                                                                            $area_name = 'HPAC Section';
                                                                            $sources_name = 'HPAC4-HP1000';
                                                                        }
                                                                    ?>
                                                                        <th class="text-center"><?php echo $sources_name; ?>
                                                                        </th>
                                                                    <?php } ?>
                                                                    <th class="text-center">Sub Total</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                <?php
                                                                // Use one table to display all data for both selected dates
                                                                $allHours = array(); // Track which date-hour combinations have been displayed
                                                                $subtotals = array(); // Initialize subtotals for each tag

                                                                // Initialize subtotals for each tag
                                                                foreach ($checkboxes as $tag) {
                                                                    $subtotals[$tag] = 0;  // Set initial subtotal for each tag to 0
                                                                }

                                                                // Loop through each day and its respective hours and display them continuously in one table
                                                                foreach ($hourlyTagConsumption as $tagData) {
                                                                    foreach ($tagData as $day => $hoursData) {
                                                                        foreach ($hoursData as $hour => $consumption) {
                                                                            // Combine date and hour to display in a single column
                                                                            $formattedDateTime = $day . ' ' . str_pad($hour, 2, '0', STR_PAD_LEFT) . ':00';

                                                                            // Check if this date-hour combination has already been displayed
                                                                            if (in_array($formattedDateTime, $allHours)) {
                                                                                continue; // Skip duplicate entries
                                                                            }
                                                                            if ($start_date == $end_date1 && $day == date('Y-m-d', strtotime($start_date . ' +1 day'))) {
                                                                                // Skip this day since it falls into the next date
                                                                                continue;
                                                                            }
                                                                            // Add the current hour to the tracking array
                                                                            $allHours[] = $formattedDateTime;

                                                                            // Display the date and hour
                                                                            echo '<tr>';
                                                                            echo '<td class="text-center">' . $formattedDateTime . '</td>';

                                                                            $rowTotal = 0; // Initialize horizontal sum for each row

                                                                            // Display the energy consumption data for each tag
                                                                            foreach ($checkboxes as $tag) {
                                                                                $fullTag = $tag . '_ActiveEnergy_Delivered_kWh';
                                                                                $consumptionValue = isset($hourlyTagConsumption[$fullTag][$day][$hour]) ? $hourlyTagConsumption[$fullTag][$day][$hour] : 0;
                                                                                echo '<td class="text-center">' . $consumptionValue . '</td>';

                                                                                // Add the current consumption to the subtotal and row total
                                                                                $subtotals[$tag] += $consumptionValue;
                                                                                $rowTotal += $consumptionValue; // Accumulate horizontal sum for this row
                                                                            }

                                                                            // Display the horizontal sum for this row
                                                                            echo '<td class="text-center"><strong>' . round($rowTotal, 2) . ' kWh</strong></td>';

                                                                            echo '</tr>';
                                                                        }
                                                                    }
                                                                }
                                                                ?>

                                                                <!-- Display the subtotal row at the bottom -->
                                                                
                                                                <tr class="text-center">
                                                                    <td class="text-center" style="font-weight:bold">Total</td>
                                                                    <?php
                                                                    // Initialize overall total
                                                                    $overallTotal = 0;

                                                                    // Loop through each checkbox (each tag) and display subtotal
                                                                    foreach ($checkboxes as $tag) {
                                                                        $fullTag = $tag . '_ActiveEnergy_Delivered_kWh';
                                                                        $subtotal = isset($subtotals[$tag]) ? $subtotals[$tag] : 0;
                                                                        echo '<td class="text-center"><strong>' . number_format($subtotal, 2) . ' kWh</strong></td>';
                                                                        $overallTotal += $subtotal;
                                                                    }

                                                                    // Display the overall total at the end
                                                                    echo '<td class="text-center" style="font-size:larger;font-weight:800;background-color:#164ebc;color: white">
          <strong>' . number_format($overallTotal, 2) . ' kWh</strong></td>';
                                                                    ?>
                                                                </tr>
                                                            </tbody>


                                                        </table>
                                                    </div>
                                                    <hr>
                                                </div><br>
                                                <?php
                                                date_default_timezone_set("Asia/Karachi");
                                                $date = isset($_GET['date']) ? $_GET['date'] : date('Y-m-d'); ?>
                                                <table width="100%">
                                                    <tr>
                                                        <td style="border-top: 0px !important;" width="30%">
                                                            <?php
                                                            echo "Generated on: " . date("h:i") . ", " . "Date:" . " " .  date("Y-m-d");
                                                            ?>
                                                        </td>
                                                        <td style="border-top: 0px !important;" width="20%">Generated
                                                            By: <b>Jahaann Technologies</b></td>
                                                        <td style="border-top: 0px !important;" width="10%" align="right">Email: <b><a href="mailto:info@jahaann.com">info@jahaann.com</a></b>
                                                        </td>
                                                    </tr>
                                                </table>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div id="styleSelector">
                </div>
            </div>
        </div>
    </div>
    <?php include('includes/footer.php'); ?>
    <script type="text/javascript" src="https://unpkg.com/xlsx@0.15.1/dist/xlsx.full.min.js"></script>
    <script type="text/javascript">
        function ExportToExcel(type, fn, dl) {
            var elt = document.getElementById('testTable');
            var wb = XLSX.utils.table_to_book(elt, {
                sheet: "sheet1"
            });
            return dl ?
                XLSX.write(wb, {
                    bookType: type,
                    bookSST: true,
                    type: 'base64'
                }) :
                XLSX.writeFile(wb, fn || ('Energy_Usage.' + (type || 'xlsx')));
        }
    </script>
    <script type="text/javascript">
        var headerImgData;
        headerImgData =
            'data:image/png;base64,';

        function generate() {
            var doc = new jsPDF('l', 'pt', 'a4', );


            // header of pdf 
            var header = function(data) {
                doc.setFontSize(18);
                doc.setTextColor(40);
                doc.setFontStyle('normal'); //margin,width , height  
                doc.addImage(headerImgData, 'JPEG', data.settings.margin.left, 20, 70, 80);
                doc.setTextColor(100);
                doc.text(570, 65, 'Naubahar Bottling Company', 'right');
                doc.setDrawColor(0, 0, 0);
                doc.line(10, 10, 840 - 10, 10);
                doc.line(10, 10, 10, 120 - 10);
                doc.setLineWidth(2);
                doc.line(10, 120 - 10, 840 - 10, 120 - 10);
                doc.setLineWidth(1);
                doc.line(840 - 10, 120 - 10, 840 - 10, 10);

                doc.line(10, 150 - 10, 840 - 10, 150 - 10);


                // get date 
                var today = new Date();
                var dd = today.getDate();

                var mm = today.getMonth() + 1;
                var yyyy = today.getFullYear();

                today1 = dd + '/' + mm + '/' + yyyy;


                // get time in 12 h format 
                var today = new Date();
                var hours = today.getHours();
                var minutes = today.getMinutes();
                var ampm = hours >= 12 ? 'PM' : 'AM';
                hours = hours % 12;
                hours = hours ? hours : 12; // the hour '0' should be '12'
                minutes = minutes < 10 ? '0' + minutes : minutes;
                strTime = hours + ':' + minutes + ' ' + ampm;
                doc.setFontSize(10);
                doc.setFontStyle('normal');
                doc.text(strTime, 730, 125);
                doc.text("Reported Date:  " + today1, 605, 125);
            };
            // page content headings design 
            doc.setFillColor(0, 122, 239);
            doc.rect(20, 150, 790, 20, 'F');
            doc.setFontSize(16);
            doc.setTextColor(255, 255, 255)
            doc.text(150, 165, 'Hourly Energy Usage Report', 'center');



            // page content borders 
            doc.setLineWidth(1);
            doc.setDrawColor(0, 0, 0);
            //left border
            doc.line(10, 550, 10, 120 - 10);
            //right border
            doc.line(840 - 10, 550, 840 - 10, 120 - 10);
            //bottom border
            doc.line(10, 550, 840 - 10, 550);
            var footer = function(data) {

                // fonts of footer texts
                doc.setFontSize(10);
                doc.setFontStyle('normal');

                // single line specs for footer border
                doc.setLineWidth(3);
                doc.setDrawColor(0, 0, 0);
                doc.line(10, 560, 840 - 10, 560);

                // get page numbers 
                const pageCount = doc.internal.getNumberOfPages();

                // For each page, print the page number and the total pages
                for (var i = 1; i <= pageCount; i++) {
                    // Go to page i
                    doc.setPage(i);
                    //Print Page 1 of 4 for example
                    doc.text('Page ' + String(i) + ' of ' + String(pageCount), 530, 820);
                } // get date 
                var today = new Date();
                var dd = today.getDate();

                var mm = today.getMonth() + 1;
                var yyyy = today.getFullYear();

                today1 = dd + '/' + mm + '/' + yyyy;

                // get time in 12 h format 
                var hours = today.getHours();
                var minutes = today.getMinutes();
                var ampm = hours >= 12 ? 'PM' : 'AM';
                hours = hours % 12;
                hours = hours ? hours : 12; // the hour '0' should be '12'
                minutes = minutes < 10 ? '0' + minutes : minutes;
                strTime = hours + ':' + minutes + ' ' + ampm;
                doc.setTextColor(0, 0, 0);
                doc.text("Generated on: " + today1, 10, 580);
                doc.text(strTime, 140, 580);
            }
            var options = { // variable for autotable for table specs 

                beforePageContent: header,

                afterPageContent: footer,

                margin: {
                    top: 200,
                    left: 20,
                    right: 20,
                    bottom: 45
                },
                //drawHeaderCell: function (cell, data) {        >>>>>>>>>>>controling individual column
                //  console.log(cell.text[0]);

                //  if (cell.text[0] ==='No.') {//paint.Name header red
                // cell.styles.fontSize= 12;
                //   cell.styles.textColor = [255,255,255];
                //   cell.styles.fillColor = "#43a089";
                //   console.log('pop');
                // } else {
                //     cell.styles.textColor = 255;
                //     cell.styles.fontSize = 10;

                // }
                //},


                startY: false,
                theme: 'grid',
                tableWidth: 'auto',
                columnWidth: 'wrap',
                showHeader: 'everyPage',
                tableLineColor: 200,
                tableLineWidth: 2,
                columnStyles: {
                    0: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    1: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    2: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    3: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    4: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    5: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    6: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    7: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    8: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    9: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    10: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    11: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    12: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    13: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    14: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    15: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    16: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },
                    17: {
                        columnWidth: 'auto',
                        halign: 'center'
                    },


                },
                headerStyles: {
                    //   theme:'plain',
                    //header fontsize
                    halign: 'center',
                    fontSize: 12,
                    textColor: [0, 0, 0],
                    fillColor: [62, 161, 255],
                },
                footerStyles: {
                    theme: 'plain',
                    halign: 'center'
                },
                // drawRow: function (cell, data) {


                //    if(odd_no<17){
                //        //console.log(data.table.rows[odd_no]);
                //        if(data.row.index===odd_no){
                //             for(i=0; i<=18 ; i++){  
                //            // data.row.cells[i].styles.fillColor = [180,218,255];

                //             }
                //            odd_no=odd_no+2;
                //        }


                //    }


                //let a = cell.textPos;
                //console.log (a.x);
                // if (cell.textPos.length ===0) {console.log('opp')}

                // },
                //willDrawCell: function (data) {
                //console.log(data.table.body);
                //var rows = data.table.body;
                //if (data.row.index === rows.length - 1) {
                //   doc.setFillColor(239, 154, 154);
                //}
                //},
                styles: {
                    //table font size
                    overflow: 'linebreak',
                    columnWidth: 'wrap',
                    font: 'arial',
                    fontSize: 11,
                    textColor: [0, 0, 0],
                    //fillColor: [180,218,255],
                    cellPadding: 8,
                    overflowColumns: 'linebreak',
                    halign: 'right'
                },
            };



            let odd_no = 0;
            var res1 = doc.autoTableHtmlToJson(document.getElementById("testTable"));

            doc.autoTable(res1.columns, res1.rows, options);

            doc.save(" Hourly Energy_Usage_Report" + today1 + ".pdf");
        }
    </script>
    <script type="text/javascript" src="./js_libraries/jspdf.debug.js"></script>
    <script type="text/javascript" src="./js_libraries/jspdf.plugin.autotable.js"></script>
</body>

</html>